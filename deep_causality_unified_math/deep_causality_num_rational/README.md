...
`0.3`; a binary float manages neither.
...
Every value is held in canonical form, and the fields are private to keep it so:
...
The fourth concerns the machine, not ℚ. `T::MIN` has no representable negation (`-i64::MIN` is
`2⁶³`, one past the top of the range), so a numerator holding it would make `Neg` partial, and with
it subtraction and the `AbelianGroup` law `a + (-a) = 0`. Construction refuses that one value per
width instead. This costs `Rational::<i64>::from_integer(i64::MIN)` and buys a negation that cannot
fail. The represented set stays symmetric, so the field laws hold.
...
`Rational<T>` is built over `T: EuclideanDomain + SignedInt`. The Euclidean domain supplies the
`gcd` that reduction to lowest terms needs. `SignedInt` supplies the rest of canonical form: a
**total** order, because `Ord` cannot answer "incomparable"; a sign, because invariant 1 moves it
into the numerator; and the end of the range, because `T::MIN` is the one value whose negation does
not fit, and the type must detect it. Every signed width qualifies: `i8`–`i128`, `isize`.

It is a [`Field`](../deep_causality_algebra/README.md) and **not** a `Real`. `Real` is the
analytic axis (`sqrt`, `exp`, `ln`, `sin`), and ℚ is closed under none of them. ℚ is
arithmetically complete and analytically empty, the mirror of `Dual`, which is analytic without
being a field.

ℤ cannot take this step on its own. Integer `/` truncates rather than inverts, so the integers
stop at `CommutativeRing`; constructing ℚ supplies the missing inverses.

## Overflow

`T` is fixed-width, so not every exact result is representable. The lists below state exactly
...
**Partial (can overflow under these conditions):**

- **Addition and subtraction.** Each operand is split into an integer part and a proper fraction,
  `a/b = q + r/b`, and the two are carried separately, so a sum with a large integer part does
  not overflow on the numerator: `MAX/2 + MAX/2` is exactly `MAX`, where forming `MAX + MAX`
...
When an operation overflows, the behaviour is `T`'s (a panic in debug builds, wrapping in
release), except where the type detects the failure itself; there it panics with a message naming
the cause. Choose `T` with headroom (`i128` where denominators are unpredictable), and
prefer accumulating over a common denominator where the problem admits one.

## Verification

The field laws, canonical-form invariants 1 and 2, and density are machine-checked in Lean 4 against
