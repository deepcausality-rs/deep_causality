...
fn test_pure_result() {
    ...
    // Let's check `deep_causality_unified_math/deep_causality_haft/src/lib.rs` or `mod.rs` to see `ResultWitness`.
    ...
    //
    // Let's actually Look at `deep_causality_unified_math/deep_causality_haft/src/hkt/mod.rs` or similar if I can, but I saw `applicative_tests.rs` working.
