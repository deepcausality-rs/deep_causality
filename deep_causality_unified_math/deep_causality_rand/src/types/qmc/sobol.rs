...
//! Sobol low-discrepancy sequence with an optional seeded digital shift.
//!
//! A point's coordinates are a deterministic function of its index (and dimension), so a given
//! index is reproducible regardless of call order. The `i`-th 32-bit Sobol value in a dimension
//! is `⊕_{k ∈ bits(g)} V[k]` where `g = i ⊕ (i >> 1)` is the Gray code of `i` and `V` are that
//! dimension's direction numbers; the coordinate is that value scaled into `[0, 1)`.
//!
//! The direction numbers below were extracted from SciPy's `scipy.stats.qmc.Sobol` (32-bit,
...
/// Number of bits / direction numbers per dimension (32-bit Sobol → period `2^32`).
const BITS: usize = 32;

/// Scale that maps a 32-bit integer into `[0, 1)`.
const INV_TWO_POW_32: f64 = 1.0 / 4_294_967_296.0;

/// A Sobol sequence over `dim` dimensions, optionally digitally shifted.
#[derive(Clone, Debug)]
pub struct SobolSequence {
    dim: usize,
    /// Per-dimension digital shift (XORed into each value). All zero when unshifted.
    shift: [u32; MAX_SOBOL_DIM],
}

impl SobolSequence {
    /// Creates an unshifted Sobol sequence over `dim` dimensions (`1..=MAX_SOBOL_DIM`).
    pub fn new(dim: usize) -> Result<Self, RngError> {
        Self::check_dim(dim)?;
        Ok(Self {
            dim,
            shift: [0; MAX_SOBOL_DIM],
        })
    }

    /// Creates a Sobol sequence with a per-dimension digital shift derived deterministically
    /// from `seed` (randomized QMC). The same seed reproduces the same shifted sequence.
    pub fn new_shifted(dim: usize, seed: u64) -> Result<Self, RngError> {
        Self::check_dim(dim)?;
        let mut rng = Xoshiro256::from_seed(seed);
        let mut shift = [0u32; MAX_SOBOL_DIM];
    ...
    pub fn coordinate<T: RandFloat>(&self, index: u64, dim_index: usize) -> T {
        ...
        let directions = &DIRECTION_NUMBERS[dim_index];

        let mut value: u32 = 0;
        let mut bits = gray & 0xFFFF_FFFF;
        while bits != 0 {
...
/// Direction numbers `V[dim][k]` (rows = dimensions, columns = the 32 direction numbers),
/// extracted from `scipy.stats.qmc.Sobol(d, scramble=False, bits=32)`.
#[rustfmt::skip]
const DIRECTION_NUMBERS: [[u32; BITS]; MAX_SOBOL_DIM] = [
