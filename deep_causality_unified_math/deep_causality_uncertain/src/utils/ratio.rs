...
/// `UncertainScalar` and not `Real` is the bound on this crate.
///
/// # Errors
///
...
pub(crate) fn ratio<R: UncertainScalar>(hits: usize, total: usize) -> Result<R, UncertainError> {
    if total == 0 {
        return Ok(R::zero());
    }
...
pub(crate) fn lift_count<R: UncertainScalar>(n: usize) -> Result<R, UncertainError> {
