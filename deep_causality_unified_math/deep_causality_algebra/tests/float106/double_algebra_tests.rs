...
fn test_realfield_ln() {
    let x = Float106::from(std::f64::consts::E);
...
fn test_realfield_log() {
    let x = Float106::from(100.0);
    let base = Float106::from(10.0);
...
fn test_realfield_log2() {
    ...
    let one = Real::log2(Float106::from(1.0));
...
fn test_realfield_log10() {
    ...
    assert!((result.hi() - 3.0).abs() < 1e-10);
    let one = Real::log10(Float106::from(1.0));
    assert!(one.hi().abs() < 1e-10);
}
...
fn test_realfield_is_nan() {
    ...
    assert!(!<Float106 as Real>::is_nan(Float106::from(1.0)));
}
...
fn test_realfield_is_infinite() {
    ...
    assert!(!<Float106 as Real>::is_infinite(Float106::from(1.0)));
}

#[test]
fn test_realfield_is_finite() {
    assert!(<Float106 as Real>::is_finite(Float106::from(1.0)));
    let inf: Float106 = <Float106 as deep_causality_num::Float>::infinity();
