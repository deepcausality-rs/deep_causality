...
#[test]
fn test_flat_tensor() {
    let flat: CurvatureTensor<f64> = CurvatureTensor::flat(4);
    assert!(flat.is_flat());
...
fn test_from_generator() {
    let tensor: CurvatureTensor<f64> = CurvatureTensor::from_generator(
        2,
        Metric::Euclidean(2),
...
fn test_weyl_tensor_size() {
    ...
    let tensor: CurvatureTensor<f64> = CurvatureTensor::flat(3);
    let weyl = tensor.weyl_tensor();
...
fn test_new_constructs_with_shape() {
    ...
    let t: CurvatureTensor<f64> = CurvatureTensor::new(
        components,
        Metric::Euclidean(2),
...
fn test_symmetry_variants_distinct() {
    ...
    let t_riemann: CurvatureTensor<f64> = CurvatureTensor::flat(2);
    assert_eq!(t_riemann.symmetry(), CurvatureSymmetry::Riemann);

    let t_weyl: CurvatureTensor<f64> = CurvatureTensor::from_generator(
        2,
        Metric::Euclidean(2),
    ...
    let t_ricci: CurvatureTensor<f64> = CurvatureTensor::from_generator(
        2,
        Metric::Euclidean(2),
...
fn test_kretschmann_scalar_with_single_component() {
    ...
    let t: CurvatureTensor<f64> = CurvatureTensor::from_generator(
        2,
        Metric::Euclidean(2),
        CurvatureSymmetry::None,
        |d, a, b, c| {
...
fn test_kretschmann_scalar_with_metric_invalid_size_returns_zero() {
    let t: CurvatureTensor<f64> = CurvatureTensor::flat(2);
...
fn test_einstein_tensor_size_3d() {
    let t: CurvatureTensor<f64> = CurvatureTensor::flat(3);
