...
///
/// The witness-level [`Endomorphism`](deep_causality_haft::Endomorphism) combinators iterate a
/// `fn`-pointer morphism. A stepper that captures `dt` and a rate field is not a `fn` pointer,
/// so — exactly as `arrow-strength` realized composition at the value level — these combinators
/// iterate a value-level endo-arrow instead. The three methods are the three integration modes:
/// fixed horizon, steady state, and integrate-until-event.
///
/// Blanket-implemented for every `Arrow<In = S, Out = S>`, so [`Euler`](crate::Euler) /
/// [`Rk4`](crate::Rk4) (and any other endo-arrow) gain them for free.
