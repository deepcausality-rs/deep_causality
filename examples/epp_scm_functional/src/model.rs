...
// f(U_smoking) -> Smoking
pub(crate) fn smoking_logic(nicotine_obs: EffectValue) -> PropagatingEffect {
...
// f(Smoking) -> Tar
pub(crate) fn tar_logic(is_smoking: EffectValue) -> PropagatingEffect {
    let mut log = CausalEffectLog::new();
...
pub(crate) fn cancer_logic(
    has_tar: EffectValue,
    has_genetic_predisposition: bool,
) -> PropagatingEffect {
    let mut log = CausalEffectLog::new();
    log.add_entry(&format!("Has tar in lung {}", has_tar));
