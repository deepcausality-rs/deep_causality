...
/// The two ways a folding step can fail.
///
/// `PhysicsError` and `CausalTensorError` both report their own reasons, and this enum carries
/// either out of `main` behind one `std::error::Error`.
#[derive(Debug)]
pub enum FoldingError {
    Physics(PhysicsError),
    Tensor(CausalTensorError),
}

impl core::fmt::Display for FoldingError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Physics(e) => write!(f, "folding step: {e:?}"),
            Self::Tensor(e) => write!(f, "folding tensor: {e}"),
        }
    }
}
...
impl From<PhysicsError> for FoldingError {
    fn from(e: PhysicsError) -> Self {
        Self::Physics(e)
    }
}

impl From<CausalTensorError> for FoldingError {
