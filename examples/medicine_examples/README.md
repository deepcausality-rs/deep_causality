...
1. **Precision is a parameter.** A `FloatType` alias sits directly above `main`, and every quantity
   in the program carries it. Change that one line to rerun the whole example at another scalar.
   All six run at `BFloat16`, `f32`, `f64` and `Float106`, and only the precision changes.
...
declares a witness, and the categorical traits are implemented against that witness. Each
example uses:
...
`fmap` applies a law to one value, `fold` reduces the payload to a number, and `extend` serves a
quantity that needs to see a neighbourhood. A reader who learns `extend` on a graph can run it on a
manifold or a point cloud, as the three `extend` examples above do.
