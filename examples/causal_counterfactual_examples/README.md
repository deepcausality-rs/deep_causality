...
Five worked examples of counterfactual intervention with the causal monad,
showing what `alternate_value` adds to `bind`.

Each follows Judea Pearl's pattern: take a factual world, substitute a
different value, and read off the difference. The intervention is a
one-shot, retrospective value substitution that defines the estimand.
...
| Audit trail in `EffectLog`. Every `alternate_value` appends `!!ValueAlternation!!: <old> replaced with <new>`, so the intervention history can be inspected. | `counterfactual_envelope_fault`, `counterfactual_cascade_failure`, `counterfactual_resection_intervention` |
...
| Comparative evaluation as the estimand. The difference between factual and counterfactual is the causal quantity of interest. | `counterfactual_treatment_effect`, `counterfactual_treatment_options`, `counterfactual_cascade_failure` |
...
Each example evaluates one chain on the factual world and on one or more
counterfactual worlds; the difference is the quantity of interest.
...
1. `counterfactual_treatment_effect` is the simplest case: the estimand
   is the difference of two interventional chains.
2. `counterfactual_envelope_fault` applies the same idea to a stateful
   `PropagatingProcess` and shows that intervention swaps the value while
   `State` and `Context` stay put.
3. `counterfactual_treatment_options` puts two interventions in the
   same chain at different sites. The intervention site encodes a causal
   claim about what lies upstream.
4. `counterfactual_cascade_failure` chains interventions: each cascade
   step applies to the result of the previous one, and the `EffectLog`
   accumulates into a forensic timeline.
5. `counterfactual_resection_intervention` shows the audit trail. In a
   clinical setting, the record of which region was resected matters as
   much as the simulator's number, and the log keeps that record.

For the closed-loop, real-time counterpart of these interventions, see
[`causal_correction_examples`](../causal_correction_examples).
...
intervention: clinical trial design, avionics, hemodynamics,
distribution-network reliability, and surgical planning.
