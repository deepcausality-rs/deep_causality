...
const PACKAGE_DIR: &str = "examples/chronometric_examples";
...
/// workspace root as `BUILD_WORKSPACE_DIRECTORY`, so neither needs anything embedded. Both
/// paths land on the same directory in the source tree, so the examples read the bundled
/// GNSS fixtures from the same place either way.
