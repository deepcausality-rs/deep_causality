...
| Audit trail in `EffectLog`. Every `alternate_value` appends `!!ValueAlternation!!: <old> replaced with <new>`. The intervention history becomes inspectable. | All five examples |
...
outside the safe envelope, `.alternate_value(corrected)` snaps it back, and
...
1. `corrective_lane_keeping` is the canonical control-loop demo. Monitor
   detects drift; P-controller produces the correction; `.alternate_value`
