...
These examples show high-assurance avionics software patterns built with `deep_causality`, focused on **Safety Critical Systems**, **Guidance, Navigation & Control (GNC)**, and **Autonomous Interventions**.
...
| [viv_resonance_margin](cfd/viv_resonance_margin/README.md) | Structures / Aeroelasticity | Vortex-induced-vibration margin as a computed study: an airspeed schedule as the case axis, one validated isolated-cylinder wake case marched per airspeed, the shedding frequency extracted from each wake probe's developed tail, and the margin to the structural mode tabled and verdicted |
| [ins_gnss_blackout](navigation/ins_gnss_blackout/README.md) | Navigation / Timing | INS clock holdover through a GPS-denial blackout (jamming / urban canyon / tunnel) on **real Galileo** data: a grmhd-style regime detector flips GNSS available↔denied, the `alternate_value` loop corrects the INS when up and is *withheld* through the dark, and the relativistic clock kernel is *carried* across the outage, in one auditable `CausalFlow` |

> **The three plasma-blackout examples tell one story.** The corridor produces
...
> shared machinery.
>
> **CFD/MMS verification examples.** The Taylor–Green MMS, Re-1600 DEC solver, lid-driven cavity, graded-MMS, and cylinder cases are self-verifying examples of the `deep_causality_cfd` crate under `deep_causality_cfd/verification/`; run them with `cargo run -p deep_causality_cfd --example <name>_verification`.
>
...
The `geometric_tcas` example shows the **Closed Loop Intervention** pattern. For safety overrides such as auto-pilot engagement, it uses the formal `AlternatableValue` trait (Pearl's Layer 2) in place of ad-hoc conditional logic.
...
This separates the **Natural History** (pilot did nothing) from the **Forced History** (auto-pilot took over) and gives "Black Box" recorders an audit trail.
...
*   **TCAS**: Uses the bivector magnitude $ \|P \wedge V\| $ to calculate impact parameters directly.
...
4. Register in `Cargo.toml`:
   ...
   ```
5. Add a matching `rust_binary` target to `BUILD.bazel`; `make check_examples` fails without one.
