...
This example computes the margin between vortex-shedding frequency and a structural mode, a
standard installation check for masts, booms, and struts. A circular member in a stream sheds a
von Karman street, which pushes on the structure at the shedding frequency `f = St * V / D`.
When that frequency approaches a structural natural mode, the member locks in and shakes itself
apart.
...
The run takes about two and a half minutes: four wake marches, run concurrently under the
...
2. `.case(model_config::wake_case).march()` runs one wake march per airspeed on the
   isolated-cylinder configuration, each through `CfdFlow::march(..).run_owned()`. The
   configuration reuses the grid, body, and probe placement of the cylinder wake verification,
   so the example stays inside validated territory.
...
The solver is validated for cylinder shedding in the laminar-wake regime, and the sweep stays
inside it (Re 100 to 160). The example does not model turbulent shedding; a flight-Reynolds strut
sits far above this range and waits on the staged turbulence work. The structural frequency and
the margin placard are demonstration values in [`constants.rs`](constants.rs), not measured
...
| [`model.rs`](model.rs) | Domain logic: the margin row, its reduction, and the three gates |
