...
and the vehicle goes dark. NASA's RAM-C II flight measured this in 1970, and the examples gate against that flight
anchor.

Blackout forces four things to hold at once:

* **The physics has to be real.** Evolved chemistry *predicts* the blackout window; no onset constant exists anywhere
  in the corridor.
...
* **Everything has to be auditable.** One provenance log carries regime transitions, nav-mode changes, bounded
  corrections, and alternation markers, and each run gates itself against that log.
...
denial, and a cybernetic gate whose clamped command steers the 3-DOF lift. Its counterfactual is a **parameter** fork:
seventeen bank-angle branches across two rounds resolve the optimum to half a degree.
...
fork copies the flow state itself.
...
Precision is a parameter throughout. Each example carries its own alias and makes the choice for itself:
