...
Counterfactual reasoning asks the rung-3 question: "given the world as it actually is, what would have happened if X had been different?" Answering it requires holding the *actual* world fixed while changing one thing in it.

Pearl's Structural Causal Model answers it in three steps: **abduction → action → prediction**.

- **Abduction** is the inference step. An SCM expresses the world as a fixed graph of variables with hidden exogenous noise. To pin down "the world as it actually is," you must *infer* the noise from the factual observations: compute the posterior over the unobserved noise variables given the observed data. This step is the known bottleneck. For general non-linear or neural SCMs it is often intractable, and much of the deep-causal-model literature (e.g., Pawlowski, Castro & Glocker, NeurIPS 2020) works around it.
...
DeepCausality **separates the causal law (the `Causaloid` or the `bind` closures) from the world state (the `Context`).** With the world state an explicit data structure rather than implicit noise, the counterfactual question becomes: *clone the world, change a piece of it, re-evaluate the same causal law*.
...
Both approaches produce identical numbers for the same problem; the choice depends on which fits your team and codebase. The five worked examples give concrete data points.
...
- The world state is heterogeneous and benefits from typed `Contextoid` kinds (`Datoid` + `Spaceoid` + `Tempoid` + `SpaceTempoid`).
...
The two approaches share the carrier (`PropagatingEffect` / `PropagatingProcess`), so one system can mix them per stage. A Causaloid graph emits a `PropagatingEffect` that a `bind` chain consumes; a `bind` chain produces input for a Causaloid evaluation. The [flight envelope monitor](../avionics_examples/control/flight_envelope_monitor/) does this: a Causaloid collection over five sensor checks, a three-step `bind` chain for state estimation, and a Causaloid hypergraph of six envelope protections, all running through one `PropagatingProcess<T, FlightState, AircraftConfig>` with state and audit log threaded across every stage.

If a system's high-level structure is a graph but its nodes contain sequential reasoning, the hybrid often fits: graph at the top, `bind` chains inside.
