...
fn main() {
    ...
    let triplets = vec![
        ...
        (0, 2, lift::<FloatType>(2.0)),
        (1, 1, lift::<FloatType>(3.0)),
    ];
    let matrix = CsrMatrix::from_triplets(2, 3, &triplets).expect("three triplets inside a 2x3");
    let original = matrix.clone();
    let doubled = CsrMatrixWitness::fmap(matrix, |x| x * lift::<FloatType>(2.0));
    ...
    let func_ptr: fn(FloatType) -> FloatType = |x| x + lift::<FloatType>(10.0);
    ...
    // measurement.
    let expanded = expand_each(&pure_mat, |x| [x, x + lift::<FloatType>(0.1)]);
    ...
    let summed_context = CsrMatrixWitness::extend(&expanded, |m: &CsrMatrix<FloatType>| {
        CsrMatrixWitness::fold(m.clone(), lift::<FloatType>(0.0), |acc, x| acc + x)
    });
    print_comonad(val, &summed_context);

    // 5. Foldable: the same reduction applied once, to the whole matrix.
    let total_sum = CsrMatrixWitness::fold(expanded, lift::<FloatType>(0.0), |acc, x| acc + x);
