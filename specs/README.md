...
C) When completed, move the spec document to /sepcs/implemented
...
reasonable job to articulate an understandable PR message. However, do expect to edit the message in terms of
...
In general, expect to allocate some time to hit the test coverage quota at the CI, and allocate some time to
review the CI code improvement suggestions, because some of them are actually really helpful to catch things like
idiomatic Rust code or even not so obvious issues i.e. unnecessary range checks or logic issues.

## Human Oversight

Project maintainers need to review, edit, validat all AI-assisted outputs to ensure it meets the specification, quality
standards and project convention. While Ai agents become incrisngly powerful in terms of execution increasingly complex
tasks, the experience with in the DeepCausality project has lead to the adoption of the folliwing core principles:

1) Continous review of the prject conventions encoded in AGENTS.md 
2) Continue oversight of agents work. 
3) Core design, innvovation, and all forms of ingenuity remain the sole responsiblity of the maintainer or contributor


## AI Safety 

Ongoing research in AI safety indicates that excssive mirroring of confirmation bias remains an ongoing challange. That means,
an AI agent is unlikely to challange a bad design decsion, poor practices, and overall maintains a "the user is always right"
attitude with all the problems that come with excessive agreableness. 

Therefore, contributors who seek to contribute to the DeepCausality project are explicitly ask to join the project Discord to
discuss contributions to mitigate development of non-features driven by affitmative Ai agents.

Maintainers working on the project code base are ask to load the AGENTS.md often into the agent to ensure the core dirrective
that the agent needs to be critical gets enforced. Furthermore, larger chages needs to be openly communicated in the project
Discord to ensure the community is informed and has a chance to raise issues at all stages of the development process.

## Closing thoughts

As AI progresses furhter and generative code improves in quality, the maintainers core responsibility shifts
away from code generation towrds a more stratigic role to ensure the right functionality for the target user group is
