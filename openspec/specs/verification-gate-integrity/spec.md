...
Keep the CFD acceptance gates honest: every shipped gate must be falsifiable, every numeric bound must
declare whether it is an external reference or a regression tripwire, CI must actually execute the
verification suite, and committed baseline artifacts must be complete runs.
