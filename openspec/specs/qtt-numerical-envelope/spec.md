# qtt-numerical-envelope Specification

## Purpose
The QTT solver family's admissibility contract — which configurations its constructors accept, which
they refuse, and where. The contract the DEC family enforces in `dec_ns_solver::cfl_check`, given to
the QTT family so an out-of-envelope configuration is rejected with a named limit rather than
integrated. Created by archiving change `close-qtt-solver-envelope`.
