...
## Purpose
Why `VecWitness` implements `Traversable`, recorded as a decision with its change of premise rather
than as a bare impl. An earlier decision withheld the impl because the accumulator fold was
unwritable against the `Applicative` bound while `HKT` carried a `Satisfies`/`Constraint` slot
(E0276). That machinery has since been removed, the fold compiles, and the impl is admitted. The
measurement that moving the inner bound to `Semigroupal + Pure` would cost sixteen admissible inner
witnesses to gain one still stands, and still pins the bound at `Applicative`. The spec fixes the
trait's contract, the documentation owed, and which witnesses the doctest may name.
