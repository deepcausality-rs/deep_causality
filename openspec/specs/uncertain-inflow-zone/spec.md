# uncertain-inflow-zone Specification

## Purpose

A sensor-fed inflow boundary zone for the DEC Navier–Stokes solver: an `UncertainInflowZone` tags
inflow cells fed by a `MaybeUncertain<R>` stream (precision-generic, no `R → f64` cast), gates each
step with `lift_to_uncertain`, applies the presence-confirmed `R` value as the Dirichlet inflow, and
on dropout fires a logged corrective `.intervene` fallback to the last-good / default value. The
uncertain typing is confined to the tagged patch; the solver rate/step stay `R: RealField`. Created by
