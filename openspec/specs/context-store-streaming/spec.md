# context-store-streaming Specification

## Purpose
Defines how a context is kept current from a store: `ContextStorageStream` and `ContextEvents`, the twelve `ContextEvent` variants, the fixed scope of a subscription, `Context::apply` and its routing table, and `ContextStore::subscribe`.
## Requirements
