# aperture-resolved-noslip Specification

## Purpose

Impose the immersed-body no-slip at the **wetted cut-face** of each `Cut` cell (a smooth,
aperture-resolved wall) instead of the binary staircase of solid-incident edges, so the discrete body
follows the immersed surface continuously. This sharpens separation, lowers the cylinder shedding
threshold (~24 cells/D → ~16 cells/D), and moves Strouhal/drag toward reference — serving the
"reference-quality `Re=100` validation in minutes, not hours" goal. Created by archiving change
`add-aperture-resolved-noslip`.
