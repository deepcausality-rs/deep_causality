# context-store-records Specification

## Purpose
Defines the records a `Context` projects onto — `ContextoidRecord`, `RelationRecord`, `ContextSnapshot` and the `NodeRecord` tree beneath them — with every scalar `f64`, every tick `u64` and every identifier the store's, and the `DataRecord` value tree a payload maps onto.
## Requirements
