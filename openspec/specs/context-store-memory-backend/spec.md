# context-store-memory-backend Specification

## Purpose
Defines the in-memory backend under the store crate's `utils_test` — `MemoryStorage`, `MemoryEvents`, `MemorySubstrate` and `block_on` — as a fold over its own event log, for tests on both sides of the contract.
## Requirements
