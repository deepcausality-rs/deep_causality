# context-explicit-dependency Specification

## Purpose
Defines the no-re-export contract: `deep_causality` does not re-export the context surface, so
reaching context requires a declared dependency on `deep_causality_context`. Context is opt-in, and
a dependency search therefore reports which crates use it.
