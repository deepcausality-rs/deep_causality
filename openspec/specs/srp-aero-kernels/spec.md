# srp-aero-kernels Specification

## Purpose
Supersonic-retropropulsion aerodynamics kernels in `deep_causality_physics`: the
freestream-normalized thrust coefficient and jet-to-freestream momentum-flux ratio, the digitized
Jarvinen–Adams central-nozzle preserved-drag correlation and unpowered baseline, the composed
total-axial-force and flow-regime margin, and the Cordell analytic plume-boundary geometry with
its sub-relations. Every correlation validates pointwise against its source's printed values and
rejects out-of-domain inputs rather than extrapolating.
## Requirements
