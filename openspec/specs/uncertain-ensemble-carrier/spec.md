...
# uncertain-ensemble-carrier Specification

## Purpose
An ensemble is `n` draws of one quantity, and it is carried by a container the **caller** names
rather than by a type this crate invents. `Uncertain::materialize::<W>` takes a witness and returns
`W::Type<R>`, so the ensemble arrives already carrying every categorical structure that witness
provides — and `deep_causality_uncertain` declares no container, implements no instance for one,
and depends on neither container crate.

One capability had to be added to haft to make that possible: building a rank-1 container from a
sequence, which `Foldable`, `Pure` and `Semigroupal` between them cannot do. Placing it in haft
rather than in the crate that noticed it was missing is what keeps the dependency tier unchanged.

The capability also fixes the composition rule for ensembles: two quantities drawn at the same
indices are correlated **by index**, so they combine under a positional zip and the diagonal
traversal — never under the cartesian `Traversable::sequence`, which forms the product.
