# context-store-facade Specification

## Purpose
Defines `ContextStore<S>`, the handle that puts projection and storage in one place — `reserve`, `hydrate`, `store_branch`, `create_node_via`, `hydrate_via` — together with `StoreError` and the `SubstrateContext` aliases.
## Requirements
