# calculus-field-hessian Specification

## Purpose
The forward-mode Hessian of a scalar field `Rᴺ → R` in `deep_causality_calculus`:
`DifferentiateFieldExt::hessian`, computed from the field's own scalar-generic model over nested
duals, exactly symmetric, and generic over the working precision.
## Requirements
