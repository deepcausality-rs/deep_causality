# context-crate-identity Specification

## Purpose
Defines what the `deep_causality_context` crate owns — the `Context` hypergraph, its
contextoids, the context node types and the contextuable traits — together with its dependency
envelope and the guarantee that a consumer of the causal monad can reach the typed context without
depending on `deep_causality`.
## Requirements
