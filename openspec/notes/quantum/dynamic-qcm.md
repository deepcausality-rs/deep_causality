...
| A4 | ~~The topology crate's cell complex can carry a **global vertex ordering / branching structure**~~ | **RESOLVED 2026-08-21, premise false.** The audit found the ordering already present: `Simplex` stores strictly-increasing vertices, `LatticeCell{position, orientation}` orders corners deterministically. Nothing needed designing; the invariants needed documenting | The break consequence never applied. A4 gated nothing, and the cup product shipped on top of what was already there (`add-cup-product`). The lesson generalises: **audit the types before costing a rung** |
...
- **SPEC-T2** — ~~Cup product on simplicial and cubical complexes.~~ **DONE** (`add-cup-product`). Binary and n-fold, generic over `ChainComplex`, `SplittableCell` supplying the per-family split. Pinned by the Leibniz identity against the crate's own coboundary operators, associativity, graded commutativity on cohomology, the `H¹×H¹→H²` torus pairing (`±L²`), and the 3-torus triple product (`L³`). Cubical geometry and signs follow Chen & Tata (arXiv:2106.05274, in `deep_causality_unified_math/deep_causality_topology/papers/`); the sign is the plain shuffle sign, their Eq. (26) prefactor being relative to their own coboundary convention.
