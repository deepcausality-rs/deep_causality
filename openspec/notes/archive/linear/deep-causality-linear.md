...
`../../../changes/archive/2026-08-30-add-linear-algebra-crate`, whose specs are now the
`linear-*` capabilities under `../../../specs`.

## Why this came up

The quantum gap register asks for 𝔽₂ linear algebra. `../../quantum/qcl-gaps.md` G-01 —
...
A prototype in `prototype` runs one generic `rref`, written against
...
Archived openspec changes under `../../../changes/archive` are historical records of what was
...
- `../../quantum/qcl-gaps.md` — G-01 (no 𝔽₂ linear algebra), G-02 (homology rank by f64 SVD)
- `../../../specs/neumann-poisson/spec.md` — names `deep_causality_sparse` normatively
