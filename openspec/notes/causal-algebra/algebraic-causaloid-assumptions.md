...
- **Resolves when.**
  - Q1. Is a 4th form plausible (probabilistic mixture, temporal/recursive, contextual switch)? Does adding one reopen the uniqueness proof, or is `F` designed for extension? **Partial: the reconvergence merge (∇) is a confirmed extension point (see #2); `F` must be designed for extension.** **Second confirmed generator (2026-07-09): the coproduct/direct-sum `⊕` (ArrowChoice over `Either`)** — required for causally faithful quantum reification (Lorenz & Barrett 2021 §3: sequential+tensor cannot make all no-influence relations simultaneously evident; direct sums are needed) and for classical contextual switches/case-splitting. Formalization + implementation planned as Stage 2b of [`causaloid-formalization-roadmap.md`](causaloid-formalization-roadmap.md).
- **Decision.** _(open — `F` must be extensible; the ∇ merge is the first confirmed new generator (#2), the ⊕ choice/direct-sum is the second (roadmap Stage 2b))_
