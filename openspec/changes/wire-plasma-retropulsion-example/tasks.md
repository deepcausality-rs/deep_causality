...
- [ ] 0.1 **M4 (`add-retropulsion-terminal-descent`) is archived.** Acts 2 and 4 and gates (2) and
...
- [ ] 0.2 Confirm where the ignition-window `q` bounds are enforced: `BurnEnvelope::q_min`/`q_max`
...
- [ ] 1.1 Create `examples/avionics_examples/cfd/plasma_blackout/retropulsion/` with the house file
...
- [ ] 1.2 `src/shared/constants.rs`: a propulsion section — thrust, `Isp`, initial mass and
...
- [ ] 1.3 `src/shared/world.rs`: a `powered_descent_coupling` assembler beside `corridor_coupling`,
...
- [ ] 1.4 `src/shared/world.rs`: a `powered_initial_field()` seeding `"mass"` and `"propellant"`
...
- [ ] 2.1 Act 0 (PLAN): load the weather table and interpolate at the measured departure (group 5
      delivers the loader); stamp the chosen row, the departure, and any clamp into provenance
- [ ] 2.2 Act 1 (CORRIDOR): fly the existing corridor legs unchanged with the burn stack composed
      and the throttle at zero
- [ ] 2.3 **Gate (1) corridor inheritance:** Acts 0–1 reproduce the corridor's blackout window,
...
- [ ] 3.1 Acts 2+3 as **one march call** with the burn stack carried from step 0 — a leg boundary at
...
- [ ] 3.2 Act 2 (COAST): `until` the ignition corridor — Mach band, `q` window, post-fix nav state,
...
- [ ] 3.3 Act 3 (BURN): on-axis burn; confirm the plume re-imprint fires from the **second** ignited
...
- [ ] 3.4 The centerpiece: pause mid-burn, `fork`, `branch` the five-world roster (coast, two
...
- [ ] 3.5 Score branches on trajectory-derived outcomes (terminal miss to a shared aim, propellant
      consumed, peak loads), committed rule pinned as a `const`
- [ ] 3.6 **Gates (4a) flow spread**, **(4b) sign-flip found** — its detail line must state that it
...
- [ ] 3.8 Confirm the roster continues concurrently through the existing `continue_branches` →
...
- [ ] 4.1 Cutoff at a leg boundary where the quasi-steady defense is honest; subsonic re-seed under
      its own γ and retuned `S_REF`, each re-seed logged; carrier rebuilds stay under the cap
- [ ] 4.2 Terminal leg to the altitude floor under M4's stopping-distance guidance with the throttle
      clamped by the burn envelope; the M = 1 crossing under thrust logged
- [ ] 4.3 **Gate (6) touchdown** — descent rate, miss to pad, propellant floor inside limits — and
...
- [ ] 5.1 Loader glue in the example's `model.rs`: read
      ...
      default value
- [ ] 5.2 Informed vs uninformed worlds on the same measured cold atmosphere, both
      `!!ContextAlternation!!`-marked, differing only in the dispersion row the guidance consumed
- [ ] 5.3 The row sizes real inputs: ignition margin `drift_mean + k·drift_sd`, propellant reserve
      from the same dispersion; row, departure, and any clamp stamped into provenance
- [ ] 5.4 **Gate (5) table earns its place** — material separation beyond a band earned on the first
      run; a non-separation is a reported finding, not a widened band

## 6. Gate set, capture, and docs

- [ ] 6.1 Assemble the full numbered set (0)–(9) as gate sequences merged into one verdict; **every
...
- [ ] 6.2 **Gate (9) wall-clock** at the caller against the 600 s budget (the study cannot see the
...
- [ ] 6.3 Pin every earned band from the first measured run and record the pins; convert any gate
      the physics cannot satisfy into a reported finding with the conversion recorded
- [ ] 6.4 `README.md` in the family form (SPDX header, How to Run, the acts, validation anchors,
...
- [ ] 6.5 Capture `output.txt` from a real passing release run

## 7. Verification and PR preparation (SDD)

- [ ] 7.1 `bazel test //deep_causality_cfd/...` green; the corridor and weather examples re-run
      bit-identical
- [ ] 7.2 `make format && make fix` — clippy clean without suppressions
...
- [ ] 7.5 Prepare the commit message(s) per task group and hand to the user (never commit); draft the
