...
- [ ] 1.1 Plume-region mask builder reusing the smoothed-mask codec (tanh volume-fraction
...
- [ ] 1.2 Tests: mask is rank-bounded under the cap; dequantized volume fraction within
...
- [ ] 2.1 Optional `ForcingRegion` (mask train, target conserved state, strength η) on the
...
- [ ] 2.2 Config/builder plumbing so a harness world can carry a forcing region
      (`CompressibleMarchConfigBuilder` seam or marcher-level member per D1)
- [ ] 2.3 Tests: interior converges toward the target to the penalization floor while the
...
- [ ] 2.4 Register new test files in their `mod.rs` chain; verify Bazel suite globs pick them
      up (`bazel test //deep_causality_cfd/...`)

## 3. Drag contraction observable (solvers/qtt)

- [ ] 3.1 Forebody-strip pressure-integration observable: pressure from the conserved
      components (ideal-gas closure) contracted against a strip mask (train `inner` + cell
      volume) → axial-force coefficient
- [ ] 3.2 Preserved-drag fraction as the powered/unpowered same-configuration ratio
- [ ] 3.3 Tests: contraction on a uniform-pressure field recovers the analytic strip integral;
      an unpowered run's own fraction is one within tolerance; observable series lands on the
      report

## 4. Verification: srp_drag_decrement

- [ ] 4.1 `verification/srp_drag_decrement/` binary (family layout: `main.rs`, `config.rs`,
...
- [ ] 4.2 Gates: per-point fraction within pinned band of `srp_preserved_drag_fraction`;
...
- [ ] 4.3 First measured run: pin the bands as constants (provenance comments), commit the
      representative `output.txt` beside the binary
- [ ] 4.4 Register the binary in the workspace (Cargo + Bazel) the way the sibling
      verifications are registered

## 5. Study: qtt_rank_plume (rank + fork economics)

- [ ] 5.1 `studies/qtt_rank_plume/` binary (family layout): C_T sweep recording `max_bond` in
      Cartesian and blend-metric coordinates; both series printed; cap gate (at least one
      coordinate under cap)
- [ ] 5.2 Fork-economics leg: march the imprinted layer to a mid-run pause; fork; roster —
...
- [ ] 5.3 Measurements + gates: fork sharing (`shares_fluid_with`/`shares_field_with`) hard
...
- [ ] 5.4 Degraded-but-measured path: non-structural misses print flagged findings and still
      exit zero (the verdict makes the call); structural failures exit nonzero
- [ ] 5.5 First measured run: pin the step-cost band, commit `output.txt`; register the binary
      (Cargo + Bazel) like the sibling studies

## 6. Verdict + cross-change sync

- [ ] 6.1 Author `openspec/notes/cfd-plasma-retropulsion/derisk-verdict.md` from the measured
...
- [ ] 6.2 Update the roadmap's M1 row/section to point at this change and the verdict note
- [ ] 6.3 Cross-check `plasma-retropulsion-cfd-contracts` remains consistent (shared
...
- [ ] 7.1 `cargo run --release -p avionics_examples --example plasma_blackout_corridor` — exit
      ...
      the corridor flies)
- [ ] 7.2 `bazel build //deep_causality_cfd/...` and `bazel test //deep_causality_cfd/...`
      green
- [ ] 7.3 `make format && make fix` — clippy clean without suppressions
- [ ] 7.4 `make test` and `make check` (SDD pre-PR gate)
- [ ] 7.5 Prepare the commit message(s) per task group and hand to the user (never commit);
