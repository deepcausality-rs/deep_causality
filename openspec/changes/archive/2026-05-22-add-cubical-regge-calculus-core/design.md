...
What is missing is the *geometric derivation layer*: every quantity in the proposal (cell volume, dihedral angle, deficit angle, Regge action) is currently uncomputable on a `LatticeComplex<D>` even though all the inputs are present in `CubicalReggeGeometry<D>`. The forward-looking design note [openspec/notes/CubicalReggeCalculus.md](CubicalReggeCalculus.md) lays out the full six-phase roadmap (R1–R6); this change set delivers R1–R3 — the geometric core — and stops at the boundary where the analytical core (Hodge ⋆, Lorentzian, dynamics) would begin.
