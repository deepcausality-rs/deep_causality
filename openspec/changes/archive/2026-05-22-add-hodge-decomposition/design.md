...
- **[Risk 5] `PointCloud::triangulate` is Vietoris-Rips, not Delaunay.** It builds every k+1-vertex clique whose pairwise edges all fit within the radius, regardless of whether the resulting complex is a manifold. For four coplanar corners of the unit square at radius ≥ √2 it produces 6 edges and 4 overlapping triangles (every 3-clique), which is not a 2-manifold and is rejected by `Manifold::with_metric`. The H3 ambient-dimension cap on `triangulate` removes one related bug (degenerate higher simplices collapsing lumped-mass M_0) but does not fix the Vietoris-Rips → non-manifold gap.
  ...
  → **Update (2026-05-26, post-archive):** the silent-zero half of this risk is closed by `harden-simplicial-hodge-degeneracy-detection`. The `triangulate` lumped-mass loop no longer silently zeroes degenerate top simplices; the rejection moved to a fallible `SimplicialComplex::hodge_star_operators` accessor with a discriminating `TopologyError::PointCloudError`. The Vietoris-Rips → non-manifold gap itself is now also closed by `add-pointcloud-delaunay-triangulation` (Block D1, 2026-05-26): `PointCloud::triangulate_delaunay` produces the manifold-respecting two-triangle unit square via Bowyer-Watson. The cross-backend test fixture switched from the single-right-triangle to the Delaunay unit square in Block D2; the strict scenario from `spec.md` 4.4 landed as a revised per-component fractional agreement at 1e-6 (the raw-norm equality variant turned out to be unachievable on coarse discretizations with different edge counts on the two sides — see `add-pointcloud-delaunay-triangulation/specs/pointcloud-delaunay-triangulation/spec.md` MODIFIED Requirement for the resolution).
