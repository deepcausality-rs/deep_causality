...
A generic kernel should bind on the algebra it actually uses. `solve_gm_analytical_kernel` uses `+ − × ÷ .abs() From<f64>` and ordering — all available on `Real + Div`, none requiring the full `RealField`. Relaxing `RealField → Real + Div<Output = R> + From<f64>` is behavior-preserving for `f64` and additively admits `Dual<f64>`, turning the kernel into a sensitivity probe (`∂GM/∂input`) at no runtime cost.

**Discovered during apply — the principle reaches the input types.** The kernel's arguments are the `RealField`-bound *public structs* `SpaceTimeCoordinate<R>` and `CentralBody<R>`, so `Dual<f64>` cannot even be constructed until those are widened too. The fix is the same principle one level out: relax both structs (and their impls) to `R: Real + Div<Output = R>` — a contained, source-compatible widening in `chronometric_quantities.rs`; impls that genuinely need more (`get_total_bias` uses `FromPrimitive`) keep their extra bound. This is a public-API widening beyond the proposal's original wording, taken on owner approval; it breaks no `f64` caller (a widening only admits more types). A broader bound audit across other physics kernels stays deferred to `causal-arrow-application`. Kalman / EKF kernels that genuinely need a matrix `inverse()` (true field structure) are *not* candidates and stay on `RealField`.

### D8 — `From<f64> for Dual`: a real literal is a constant dual

The kernel builds its constants with `R::from(0.5)` and the like, i.e. it bounds `R: From<f64>`. `Dual<f64>` did not implement `From<f64>`, so it could not satisfy that bound. The fix is one additive impl, `impl<T: Real + From<f64>> From<f64> for Dual<T>`, mapping a literal to `Dual::constant(T::from(x))` — value `x`, derivative `0`. This is semantically exact: a constant has zero derivative, so literals threaded through a kernel never leak into the `ε` channel. It is the single additive change to the `dual-numbers` surface (a conversion, not arithmetic), and it generalizes: any `From<f64>`-bounded generic code now accepts `Dual`.
