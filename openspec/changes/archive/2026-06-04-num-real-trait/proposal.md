...
- **Float-blanket cascade (maintenance consolidation).** Replace the per-type `f32`/`f64`/`Float106` algebra impls with **blanket impls over `Float`**, so a type that implements `Float` inherits the whole algebra tower automatically. Concretely: add `pi()`/`e()` to `Float` and strengthen its supertraits with the in-place arithmetic operators (`AddAssign`/`SubAssign`/`MulAssign`/`DivAssign`); then provide `impl<T: Float> Associative/Commutative/Distributive/AbelianGroup/DivisionAlgebra<T>/Real/RealField for T`, with `Real` delegating to the corresponding `Float` method. The intermediate traits (`Ring`, `CommutativeRing`, `Field`, `MulMonoid`, `InvMonoid`, …) are already blanket-derived, so they need nothing. The explicit `f32`/`f64`/`Float106` algebra impls are deleted (only `Zero`/`One`/`Num`, which sit *below* `Float`, remain per-type). **Result: adding a future float type (`f16`/`f128`) requires only `impl Float` — no edits across the algebra files.** Coherence is safe because `Float` is a *local* trait: the orphan rule guarantees `Complex`/`Quaternion`/`Octonion`/`Dual` can never implement it, so the blankets do not overlap their (non-`Float`) impls — verified by a `rustc` probe and the full workspace build.
- **No public API is removed and no `RealField` bound elsewhere is touched.** The change is additive (new traits/methods) plus an internal relocation of impls behind blankets and the supertrait. No external or numeric dependency is added.
