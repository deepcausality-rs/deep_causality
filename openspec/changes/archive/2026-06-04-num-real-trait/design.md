...
- **[Behavior drift during impl relocation.]** Moving method bodies could subtly change results. → Mitigation: relocate verbatim (no rewrites); keep the existing analytic tests but run them through `Real`; assert bit-identical results for `f32`/`f64`/`Float106` (4241 + 177 num tests pass).
