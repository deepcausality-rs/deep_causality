...
- **Crates touched.** Primarily `deep_causality_algorithms` (new modules under `causal_discovery::brcd`; the logistic gate and MEC engine live here too unless a reusable home is justified during design). Consumes the already-public Tier A primitives (`deep_causality_tensor`, `deep_causality_sparse`) and `deep_causality_topology::MixedGraph`. Adds a dependency on `deep_causality_rand` for seeded uniform sampling (design D8), and a small enabler there — `Xoshiro256::from_seed(u64)` + a crate-root re-export — so a BRCD run is reproducible from a seed.
