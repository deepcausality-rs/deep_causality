...
`brcd_helper` (L1863) branches: CPDAG supplied → `brcd_update` directly; none → bootstrap CPDAGs via BOSS + `get_top_k_cpdags_with_ratio` + `parallel_weighted_posterior`. This change implements the **supplied-CPDAG** branch only. The bootstrap/BOSS branch is a separate concern (CPDAG discovery is upstream; CDL produces the graph). The API accepts an optional list of weighted CPDAGs so the bootstrap path can be added later without a signature change. The author's `LocalScoreFunction.py` (BIC / BDeu / RKHS scores BOSS optimizes) belongs entirely to that deferred BOSS change — it does not touch this estimator (note §16.6).

**D6a. Driver contract (mirrors the reference `brcd_helper`).** The entry point takes the two datasets, the CPDAG, and a `BrcdConfig` carrying `node_transform`, `transform_parents`, the family kind (continuous/discrete), `num_root_causes_candidates = k`, and the seed; it returns a `BrcdResult<T>` whose `ranks` field is the **ranked candidate ordering** (best first), matching the reference's `result["ranks"]` (README §"Run BRCD"). `transform_parents` applies the node's effective transform to continuous parents with **no Jacobian** (brcd.py L409–421), and is a no-op when `node_transform="none"`.
...
**D8a. Module conventions (engineering discipline).** Every source file under `causal_discovery::brcd` carries a `brcd_` prefix (`brcd_mec`, `brcd_gate`, `brcd_gaussian`, `brcd_dirichlet`, `brcd_meek`, `brcd_validity`, `brcd_linalg`, `brcd_error`) so files are self-identifying repo-wide. **All fallible BRCD operations return the single [`brcd_error::BrcdError`]** — a newtype `BrcdError(BrcdErrorEnum)` with `Display` + `Error`. The error *type* is fixed in every signature; new failure *cases* are added as `BrcdErrorEnum` variants as the implementation grows, without churning any signature (mirrors the repo's `TopologyError(TopologyErrorEnum)` idiom). Stages 5–10 add variants, never new error types.
...
**Mechanism (owner directive): runnable Rust examples, not test fixtures.** Each verification is a standalone example under `examples/verification/`, declared as a named `[[example]]` in `Cargo.toml` and run individually (`cargo run --example <name>`); each prints `PASS`/`FAIL` and exits non-zero on failure. Shared CSV/CPDAG/reporter helpers live in `examples/verification/common.rs`; the Python→CSV→expected workflow is documented in `examples/verification/README.md`. The unit tests under `tests/` already cover the algorithm; the examples are the *verification* deliverable.

**The acceptance bar is tier 3 — reproducing the real-world experiment rankings** (OB / Sock Shop), delivered as the `real_world_*` examples that replay committed Python-derived CSV + expected ranks. The base and synthetic tiers are smoke tests that gate progress cheaply; they are necessary but not sufficient. Three tiers, fastest-to-run first:

1. **Golden unit fixtures.** Small `(normal, anomalous, CPDAG)` cases with the reference posterior over roots captured from `ctx/next/brcd/brcd.py`, committed as golden data. Assert the Rust ranking is **identical** and each log-posterior is within a tolerance `ε`. Covers each estimator mode (F∈parents, F∉parents mixture, F absent; continuous and discrete) and the MEC weighting on a CPDAG with undirected edges. **The primary fixture is the author's `X → Y → Z` toy** (`ctx/next/example.txt`): an undirected CPDAG `edges=[(X,Y),(Y,Z)]` (the path-of-3 chain component our stage-1 MEC enumerates), anomaly perturbing `p(Y|X)`, `node_transform="none"`, expected `ranks == ['Y','X','Z']`. Its data is generated by numpy PCG64 (not bit-reproducible in Rust), so the generated `df_obs`/`df_a` are committed as CSV golden *inputs* and the Rust ranking is asserted equal to `['Y','X','Z']`.
