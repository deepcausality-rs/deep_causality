...
- Add fluid-specific physical-quantity newtypes appended to the existing `deep_causality_physics/src/kernels/fluids/quantities.rs` (matching the crate convention for domain-specific units — cross-domain units like `Temperature` live in `units/`, domain-specific units like `Pressure`, `Density`, `Viscosity` live alongside their kernels):
  - **Scalar quantities (positivity / finiteness invariants):** `KinematicViscosity<R>` (m²/s), `SpecificEnthalpy<R>` (J/kg), `WallShearStress<R>` (Pa). Existing scalar newtypes (`Density`, `Pressure`, `Speed`, `Length`, `Temperature`, `Viscosity` for dynamic viscosity μ) are reused; no parallel hierarchy and no rename.
  - **Vector quantities (semantic-distinction / convention-encoding invariants):** `Velocity3<R>`, `VorticityVector<R>` (pseudovector under reflection), `AccelerationVector<R>`, `BodyForceDensity<R>` — all `[R; 3]` newtypes, finiteness-checked.
  - **Rank-2 tensor quantities:** `VelocityGradient<R>` (pins the Jacobian convention `[i][j] = ∂u_i/∂x_j` at construction), `StrainRateTensor<R>` (symmetric, checked at construction), `RotationRateTensor<R>` (antisymmetric, checked at construction), `CauchyStress<R>` (symmetric, positive in tension).
  - Each typed newtype provides `new(raw) -> Result<Self, PhysicsError>`, `new_unchecked(raw)`, `value() -> &[…]`, `into_inner() -> […]`, `From<[…]>` / `From<Self> for […]`, `Default`, and the standard derived traits. The `From` impls make typed and raw-array consumers interoperable at the call site, preserving Block B5 compatibility (`3DCausalFluidDynamics.md`) without forcing a B5 redesign.
  - This reverses the original "vectors and tensors are raw arrays" stance, which was not defensible against contemporary CFD/FEM library practice (OpenFOAM `vector`/`tensor`/`symmTensor`, deal.II `Tensor` / `SymmetricTensor`, FEniCS UFL, Eigen `SelfAdjointView`) or against the project's own precedent in `deep_causality_metric` (typed metric signatures). See `design.md` §D1 for the full rationale.
