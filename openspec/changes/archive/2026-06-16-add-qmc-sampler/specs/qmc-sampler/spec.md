...
is seeded from the active `seed_sampler` value, so that a seeded run is
reproducible. Because the shift is randomized, `standard_deviation` over QMC draws
SHALL estimate genuine sampling error (not zero). When no seed is in effect, the
shift SHALL be drawn from the OS-entropy source.

#### Scenario: Seeded QMC run is reproducible
- **WHEN** `seed_sampler` is set and a QMC batch estimate is computed twice
- **THEN** both runs produce identical estimates

#### Scenario: Standard deviation is non-degenerate under QMC
- **WHEN** `standard_deviation` is computed over a QMC batch of a non-constant integrand
...
`standard_deviation`, and `estimate_probability` through an explicit opt-in (a
dedicated method or a sampling-method argument), never as implicit global state.
The existing MC signatures and their results SHALL be unchanged for callers that
do not opt in.
...
- **WHEN** a caller opts into QMC on a batch estimator over a static low-dimension tree
