...
- **Dependencies:** uses `deep_causality_rand` (already in workspace) for Metropolis randomness; reuses `deep_causality_num::Complex<R: RealField>` for the Wick-rotated Lorentzian action. The Block 0 audit confirmed the type is already exposed at the crate root with the required `R: RealField` generic shape (see [`deep_causality_num/src/lib.rs:59`](../../deep_causality_num/src/lib.rs#L59)); no coordinated change to `deep_causality_num` is needed.
