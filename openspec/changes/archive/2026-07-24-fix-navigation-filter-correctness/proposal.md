...
- **Give the process noise a stated discretisation.** `Q` becomes a documented continuous-time
  ...
  `Q` to floating-point rounding (bit-exact for most blocks; a ≤1-ULP residue on the gyro-bias and
  clock-drift blocks, below any displayed digit). This change alone therefore does not move the example
  figures (measured); it would for a filter run at a different step size, which is the point.
...
- Re-tuning is required for the *contract*, but at the examples' fixed `dt` the dimensionally-correct
  `÷dt` re-tune is calibration-preserving, so group 4 alone leaves every displayed figure unchanged on the
  examples (measured; bit-exact bar a ≤1-ULP gyro-bias/clock-drift residue below display precision).
