# fluid-dynamics-theory Specification

## Purpose

Assemble the pointwise kernels from `fluid-dynamics-kernels` into the four classical regimes of Navier–Stokes fluid mechanics (incompressible Newtonian, compressible Newtonian, Euler inviscid, Stokes creeping). Each regime is a free function that composes published kernels — never re-derives algebra — and returns the pointwise RHS of the corresponding evolution equation in `∂/∂t = …` form, generic over `R: RealField`.
