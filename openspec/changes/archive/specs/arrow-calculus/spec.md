# arrow-calculus Specification

## Purpose

Analytic calculus over the `Arrow` algebra: forward-mode differentiation as the tangent
functor and ODE integration as endo-arrow iteration, provided by the `deep_causality_calculus`
crate. The crate depends on `deep_causality_haft` for the `Arrow`/`Endomorphism` machinery and
on `deep_causality_num` for `Dual`, so both foundations stay self-contained; the user-facing
surface follows the repo's `…Ext` type-extension convention and never makes the caller name
`Dual`, `ε`, or a seed.
