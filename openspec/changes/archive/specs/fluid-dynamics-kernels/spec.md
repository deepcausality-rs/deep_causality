# fluid-dynamics-kernels Specification

## Purpose

Provide a comprehensive, pointwise, stateless kernel surface for fluid mechanics in the `deep_causality_physics` crate — governing equations (continuity, momentum, energy, vorticity transport, scalar advection-diffusion), constitutive relations (Newtonian / power-law viscous stress), kinematics (strain rate, rotation rate, vorticity, invariants, helicity, enstrophy), dimensionless numbers, turbulence quantities, coherent-structure detectors (Q, λ₂, Δ, swirling strength), compressible-flow thermodynamics, boundary-layer relations, and ideal-flow primitives. All kernels are generic over `R: RealField`, take typed vector/tensor newtypes with construction-time invariants, and are shadowed by causal `PropagatingEffect`-returning wrappers.
