...
**Decision:** Keep `ReggeGeometry<C>` simplicial-flavored. Add a parallel `CubicalReggeGeometry<D>` (or a `MetricKind` enum at the `Manifold` level) that, for the unit-edge case, returns `1.0` for every edge length and short-circuits volume computations to integer lattice volumes. Non-uniform / scaled / curved cubical metrics are deferred.
...
- **D8-actual.** Initial Stage C rename was `Lattice<D>` → `CubicalComplex<D>` (per spec D4). On post-implementation review the rename was identified as overclaiming: the type stores only `shape` + `periodic` + a coboundary cache (i.e. the **regular ℤᴰ lattice** with cubes computed on demand), not an arbitrary cubical complex. Final state: canonical name `LatticeComplex<D>` with `CubicalComplex<D>` as a `pub type` alias. The same dual-name principle applies to `LatticeCell<D>` ↔ `CubicalCell<D>`. No other Cubical aliases ship: types such as `DualLatticeComplex<D>`, `LatticeComplexWitness<D>`, and the cell iterator have only their canonical name because they have no genuinely distinct cubic-specialization reading in the literature. Documented in `docs/TOPOLOGY.md` § "LatticeComplex / Cubical Complexes" and `README.md` § "Mathematical Naming Convention".

- **D9-actual.** Initial Stage B introduced `CubicalMetric<D>` (struct with `unit_edge: bool`) as the simplicial-counterpart metric type on the lattice path. Post-implementation review found `CubicalMetric` is not an established mathematical term (no AT or DDG literature uses the compound as a frozen name). The simplicial counterpart `ReggeGeometry<T>` is named after Tullio Regge's 1961 discrete-gravity construction; there is real (if minor) literature on "cubical Regge calculus" / "hypercubic Regge calculus" extending the same construction to cubical complexes. **Final name: `CubicalReggeGeometry<D>`**, paralleling `ReggeGeometry<T>` in role and naming convention (both are the `Metric` associated type for their respective complex, both named after the Regge construction transported to their cellular world). The type was also expanded to its forward-looking shape: edge lengths at four levels of uniformity (`UnitEdge`, `Uniform { length }`, `PerAxis { lengths }`, `PerEdge { lengths }`) plus optional Lorentzian timelike-axis flags and a Stage-C-implemented `signature()` method. Cell volumes, deficit angles, Hodge ⋆, and per-cell metric tensors are explicitly forward-looking and documented as such in the type's module doc. Folder `src/types/cubical_metric/` renamed to `src/types/cubical_regge_geometry/`. Tests: 30 unit tests covering all four representations + getters + Lorentzian signature.
