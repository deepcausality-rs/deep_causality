...
- [ ] 2.1 Add `type Metric;` to the `ChainComplex` trait in `src/traits/chain_complex.rs`. No bound on `Metric`. Update every existing impl in lock-step: `SimplicialComplex<T> → type Metric = ReggeGeometry<T>;`, `Lattice<D> → type Metric = CubicalReggeGeometry<D>;`, `CellComplex<C> → type Metric = ();`. Build between each impl.
- [ ] 2.2 Create `src/types/cubical_regge_geometry/mod.rs` defining `pub struct CubicalReggeGeometry<const D: usize> { unit_edge: bool }` plus `pub fn unit() -> Self`. Register under `src/types/mod.rs`. Re-export `CubicalReggeGeometry` from `lib.rs`.
