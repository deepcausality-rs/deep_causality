...
- [ ] 4.1 Invert the hash policy to fail-closed, with per-platform overrides and an explicit development opt-out that announces itself
- [ ] 4.2 Replace the full-history clone in the subprocess fallback with a shallow fetch at the pinned revision, retaining `.git` and the `origin` remote that the Mathlib cache client needs, and accepting both tags and bare commit hashes
- [ ] 4.3 Add a retry around `lake exe cache get` for the case where it exits zero with gaps
- [ ] 4.4 Add post-fetch completeness validation with actionable messages, replacing the bare "no oleans found" failure
- [ ] 4.5 Strip macOS quarantine attributes from downloaded toolchains
- [ ] 4.6 Keep source-building Mathlib behind an explicit opt-in rather than running it unconditionally after `cache get`
...
- [ ] 6.1 CI asserts a missing toolchain hash fails with the message that directs the consumer to supply one
- [ ] 6.2 CI verifies every declared platform's toolchain archive downloads and extracts under its recorded sha256
- [ ] 6.3 CI asserts Lean/Mathlib version skew fails with a message naming the mismatch
- [ ] 6.4 CI asserts lock drift fails: a package revision disagreeing with the manifest, and a lock entry missing its sha256
- [ ] 6.5 CI asserts a `file://` pin is rejected
- [ ] 6.6 CI covers both macOS and Linux, and exercises remote execution so the platform-selection fix stays fixed
- [ ] 6.7 CI builds the Mathlib-dependent examples, which presubmit deliberately does not

## 7. Release and submit

- [ ] 7.1 Produce the release archive from the tag so it matches what `source.template.json` resolves to
- [ ] 7.2 Wire release automation that opens the registry submission from a published release without hand-editing the integrity hash
- [ ] 7.3 Cut the first release and submit to BCR
- [ ] 7.4 Confirm presubmit passes and stays fast, having fetched no Mathlib

## 8. Migrate this repository

- [ ] 8.1 Replace the `local_path_override` on `thirdparty/rules_lean` with a `bazel_dep` on the published module, updating `use_repo` and `register_toolchains` names
- [ ] 8.2 Update `lean/BUILD.bazel`, `lean/lake-lock.json` and `lean/lake_packages.bzl` for any rule-surface change
- [ ] 8.3 Verify `bazel build --config=remote //...` is green and the Lean namespaces type-check and cache as before
- [ ] 8.4 Move `thirdparty/rules_lean/` aside per repository convention, with a note recording what superseded it
- [ ] 8.5 Prepare the commit message; leave committing to the user

## 9. Close out

- [ ] 9.1 Update `lean/README.md` with the dependency change and the repin workflow
- [ ] 9.2 Record in the proof-header note that a new import needs both a `cache_roots` update and an archive repack
- [ ] 9.3 Decide whether to offer the lock design upstream to `tomato-bazel/rules_lean` and `pulseengine/rules_lean`, and open the LICENSE request against the latter
