...
crates. Adding `deep_causality_file` to `deep_causality_discovery`'s `[dev-dependencies]` therefore
reaches Bazel with no hand edit.

`deep_causality_file` has no `utils` module. Its runtime dependencies are `chrono`, `haft`, `num`
and `algebra`.

## Goals / Non-Goals

**Goals:**
- Provide the table's surface with std only, in `deep_causality_file`.
- Keep `deep_causality_file`'s runtime dependency set and tier unchanged.
...
### D1 — Public types exported from the crate root
The types are `TempDir` and `NamedTempFile`, under `src/types/temp_dir/` and
`src/types/named_temp_file/`, and both are re-exported from `lib.rs`. Each module follows the
...
*Alternative (rejected):* put both types in a public `utils_tests` module, the convention for test
helpers shared across crates. A scratch file or directory is ordinary file-crate functionality, so
the types are a supported API and consumers import them from the root.
...
- [The new types enlarge the public API of a published crate] → The addition is non-breaking and
  small: two types and five methods.

## Migration Plan

1. Record the pre-migration test counts of the three crates under `cargo test` and `bazel test`.
2. Phases 1–5 for the two types in `deep_causality_file`.
3. Rewrite call sites crate by crate, `file`, then `cfd`, then `discovery`. After each crate, remove
   its `tempfile` dev-dependency, build and test it, and compare counts.
