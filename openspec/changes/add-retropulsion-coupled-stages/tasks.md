...
- [ ] 1.1 Add `RetroThrust` to `types/flow/retropulsion.rs` (beside `PropulsionStub`, re-exported
...
- [ ] 1.2 Depletion: reduce `"propellant"` and `"mass"` by `ṁ·Δt` (`ṁ` from
      `propellant_mass_flow_kernel`, `Isp` a stage parameter), using the start-of-step mass for the
      thrust normalization; set `"ignited"`
- [ ] 1.3 Tests (`tests/types/flow/retropulsion_tests.rs`): thrust composes onto lift (axial gains
...
- [ ] 2.1 Add `PlumeObstruction` to `types/flow/retropulsion.rs`: at throttle > 0, derive
...
- [ ] 2.2 Optional state-realism imprint behind an explicit opt-in: a `plume_mask_2d` mask from
      ...
      with or without it)
- [ ] 2.3 Tests: the applied fraction equals `srp_preserved_drag_fraction_kernel(C_T)` within the
...
- [ ] 3.1 Extend `RegimeClass` (`corridor.rs`) with a Mach regime, a thrust state, and a touchdown
      ...
      are absent
- [ ] 3.2 Fold the new axes into `key()` so a Mach crossing, a burn↔coast transition, or a touchdown
      is a logged regime change; the continuous values stay out of the key (no spurious re-logs)
- [ ] 3.3 Tests (`tests/types/flow/corridor_tests.rs`): each axis reads its scalar; the corridor
...
- [ ] 4.1 Harness test extending the M2 inheritance-guard pattern: march a corridor-class coupled
...
- [ ] 4.2 Register new/edited test files in their `mod.rs` chain and the `tests/BUILD.bazel` globs
      (`bazel test //deep_causality_cfd/...`)

## 5. Verification and PR preparation (SDD)

- [ ] 5.1 `bazel test //deep_causality_cfd/...` green; the corridor + weather examples re-run
      bit-identical (the classifier extension is inert on the corridor's scalars)
- [ ] 5.2 `make format && make fix` — clippy clean without suppressions
- [ ] 5.3 `make test` and `make check` (full-repo SDD pre-PR gate)
- [ ] 5.4 Prepare the commit message(s) per task group and hand to the user (never commit); draft the
