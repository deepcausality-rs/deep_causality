...
- [ ] 1.1 Add newtypes `ElectronDensity`, `IonizationFraction`, `ElectronTemperature`,
...
- [ ] 1.2 Register the newtypes in their `quantities/<module>/mod.rs` and flat-export via `lib.rs`.
- [ ] 1.3 Add `src/constants/hypersonic.rs` with the Park reaction `A/n/E_a` coefficients (incl. the dominant
...
- [ ] 1.4 Tests (`tests/` mirror, f64): each constructor accepts valid input and rejects each out-of-range
...
- [ ] 2.1 Create the `kernels/hypersonic/` domain (`mod.rs`, `wrappers.rs`) and flatten at `lib.rs`
  (`pub use crate::kernels::hypersonic::*;`).
- [ ] 2.2 `vibrational_relaxation_kernel<R>` — Landau–Teller increment toward `T_tr` with Millikan–White
...
- [ ] 2.3 `arrhenius_rate_kernel<R>` — `k(T) = A·T^n·exp(−E_a/(k_B T))`, forward/backward → `ReactionRate`.
- [ ] 2.4 `ionization_fraction_kernel<R>` — Saha-equilibrium and rate-based forms → `IonizationFraction` /
  `ElectronDensity`; the `α_eq` target includes NO⁺ + e⁻ so it can be nonzero.
- [ ] 2.5 `plasma_frequency_kernel<R>` — `ω_p = √(n_e e²/(ε₀ m_e))` constructing the existing
  `PlasmaFrequency` newtype (NEW kernel — `mhd/plasma.rs` lacks it; add it there, reusing the `mhd` constants).
- [ ] 2.6 `rankine_hugoniot_temperature_kernel<R>` — normal-shock post-shock temperature ratio from `M_∞`
  (and `γ`) → `Temperature` (`T_post`).
- [ ] 2.7 `recovery_temperature_kernel<R>` — `T_tr = T_post − ½|u|²/c_p` (pointwise; `|u|`, `c_p` supplied).
- [ ] 2.8 `park2t_ionization_surrogate_kernel<R>` — the Tier-A fitted `α(ρ, T, …)` equilibrium target
  (lookup-table-reduction route).
- [ ] 2.9 `PropagatingEffect` wrapper for each kernel in `wrappers.rs` (`match kernel(…){ Ok(v) =>
...
- [ ] 3.4 Recovery-temperature wiring — build the `T_tr` scalar field each step from the computed velocity
...
- [ ] 3.5 Tests (f64): LER helper equals the analytic exponential to round-off; stays bounded/monotone at
...
## 4. Blackout trigger + observables (`deep_causality_cfd`)

- [ ] 4.1 `BlackoutTrigger` on the `CausalFlow` seam (grmhd `select_metric` shape): `n_e →
  plasma_frequency_kernel →` compare to the **config** comms band → GNSS/comms-denied flag; a stage
  returning `PropagatingEffect`, adapted with `.into()` / `bind_or_error`.
- [ ] 4.2 Extend `QttObserve` with opt-in `n_e` / plasma-frequency / blackout-dwell flags and emit them as
  `Report` series (`add_series`) from the QTT march run, alongside the existing diagnostics. Reuse
  `wall_heat_flux` for the neutral thermal series.
- [ ] 4.3 Tests (f64): trigger raises the flag above the band and not below; the march run emits the `n_e`,
...
- [ ] 5.1 `config.rs` (configuration only), `main.rs` (the `CfdFlow::qtt_march` run + ionization coupling +
...
- [ ] 6.1 `make format && make fix` (both crates changed); clippy `--all-targets` clean — **fix, don't
  ...
  exit 0.
- [ ] 6.2 Confirm constraints: static dispatch only, no `dyn`/`unsafe`/lib-macros; crate-root imports; lib
...
- [ ] 6.3 `openspec validate add-park2t-blackout-tier-a --strict` passes.
- [ ] 6.4 Mark **Gap 2 Tier-A closed** in `openspec/notes/plasma-blackout/gap-analysis.md` §4 Gap 2 and the
