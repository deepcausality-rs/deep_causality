...
- [ ] 1.1 Add a flight-sensor `PhysicsStage` publishing `"q_inf"` and `"descent_rate"`, both field
...
- [ ] 1.2 `"descent_rate"` = `−(r·v)/|r|` from `"truth_state"`, **positive downward**. The sign is
...
- [ ] 1.3 Inert when `"truth_state"` is absent or `len() < 6` — publish nothing rather than a value
      from a partial state
- [ ] 1.4 Tests: both scalars published with correct units and sign; descending yields positive rate;
...
- [ ] 2.1 Add `ThrottleGuidance` beside `RetroThrust`/`PlumeObstruction`: `a_cmd = v²/2h + g` via
...
- [ ] 2.2 Write the channel on **every** step, θ = 0 before commit. The gate's burn block is guarded
...
- [ ] 2.3 Propagate the kernel's singularity error at altitude ≤ 0 rather than producing a throttle
- [ ] 2.4 Tests: the commanded θ matches the law; θ saturates at 1; θ = 0 pre-commit leaves the
...
- [ ] 3.1 The four-condition conjunction: Mach band ∧ `q∞ ∈ [q_min, q_max]` ∧ post-fix nav ∧ position
...
- [ ] 3.2 Rising-edge detection with a **one-way latch carried on a field scalar** (not
...
- [ ] 3.3 Read nav through the published `"nav_mode"` and `"nav_position_variance"` scalars, **never**
...
- [ ] 3.4 Log one commit entry naming the step and the four sensed values, transition-only (the
      nav-mode flip is the precedent)
- [ ] 3.5 Tests: all four hold → commit; each one short → no commit; commit fires exactly once across
...
- [ ] 4.1 **Enforce `[q_min, q_max]`** — today stored on `BurnEnvelope` with zero read sites and zero
...
- [ ] 4.2 **Fix the crossed clamp window.** `clamp(commanded, throttle_floor, ceiling)` has no
...
- [ ] 4.3 **Fix breach masking.** The thermal/g refusal returns before the burn block, so a heat or g
...
- [ ] 4.4 **Refuse the blind-gate case**: burn axes attached ∧ positive `"commanded_throttle"` scalar
...
- [ ] 4.5 Surface the unusable burn-sensing configuration (burn axes attached with a non-positive
...
- [ ] 4.6 Document the per-axis reduction and its safety direction: max-over-cells folded from zero is
...
- [ ] 4.7 Tests for every case listed as untested today: the q window, breach masking, `with_burn`
...
- [ ] 4.8 Re-run the corridor inheritance guard: no world that omits `with_burn` reaches any new
      refusal, so the corridor and weather examples stay **bit-identical**

## 5. Terminal leg: re-seed visibility and rebuild budget

- [ ] 5.1 Log a leg-boundary re-seed entry naming the incoming world and recording that the marched
      ...
      them
- [ ] 5.2 Add an explicit rebuild bound enforced in-loop (`Err` at the bound) **and a
...
- [ ] 5.3 Document the trigger's scope and blind spot: wave-speed-keyed, **per-carrier** (s_ref
...
- [ ] 5.4 Terminal-leg configuration: its own reference scales, acoustic reference, and seed, with
...
- [ ] 5.5 Tests: the re-seed entry appears and existing texts are unchanged; the cap refuses at the
...
- [ ] 6.1 The **first** test anywhere composing `CyberneticCorrect` **with** burn axes into a marching
...
- [ ] 6.2 Drive a coast → commit → burn → cutoff → terminal leg through the guidance stage; assert
      **gate (2) ignition corridor** (the commit fired inside the band, window, nav state, and margin)
- [ ] 6.3 Assert **gate (6) touchdown** — altitude floor reached with descent rate, miss, and
      propellant inside bounds — and the transonic crossing under thrust logged as a regime transition
- [ ] 6.4 Assert **gate (8) bounded rebuilds** against the now-explicit bound via the new accessor
...
- [ ] 6.5 Assert integrity across the leg — no step captured an error, the envelope held

## 7. Downstream reconciliation

- [ ] 7.1 Amend `openspec/changes/wire-plasma-retropulsion-example/`: the `"q_inf"`/`"descent_rate"`
...
- [ ] 7.2 Update `openspec/notes/cfd-plasma-retropulsion/plasma-retropulsion-roadmap.md` — M4 archived,
      M5 unblocked

## 8. Verification and PR preparation (SDD)

- [ ] 8.1 `bazel test //deep_causality_cfd/...` green; the corridor and weather examples re-run
      bit-identical
- [ ] 8.2 `make format && make fix` — clippy clean without suppressions
- [ ] 8.3 `make test` and `make check` (full-repo SDD pre-PR gate)
- [ ] 8.4 Prepare the commit message(s) per task group and hand to the user (never commit); draft the
