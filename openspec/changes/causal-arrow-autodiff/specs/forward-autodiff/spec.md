...
`deep_causality_num` SHALL provide multi-input forward-mode helpers over fixed-arity `const`-generic arrays. It SHALL provide `gradient::<N>(f, &[R; N]) -> [R; N]` for `f: Fn(&[Dual<R>; N]) -> Dual<R>` (one seeded coordinate per pass), `directional_derivative::<N>(f, &[R; N], &[R; N]) -> R` computing the derivative along a direction in a single pass, and `jacobian::<N, M>(f, &[R; N]) -> [[R; M]; N]` for vector-valued `f`. All SHALL be allocation-free and bounded on `R: Real + Div<Output = R>`.
