...
- [ ] 3B.10 **Found, measured, and deliberately not fixed here.** The same accumulation defect exists in `deep_causality_stats`'s multi-accumulator reductions — `correlation`, `covariance`, `ridge` and `logistic` — which run several running sums at once rather than one. Witness: `pearson` over a perfectly correlated pair returns `1.0078` at n = 100 and `1.0156` at n = 1000 at `BFloat16`, a correlation coefficient above one, which is not merely inaccurate but outside the quantity's range. It grows with n exactly as the mean's did.

      Not fixed with the others because it is a different size of change and none of it is on this change's path. The single-sum reductions took one `PairwiseSum` each; a covariance takes one per cell of a `p × p` matrix, which is `p² · usize::BITS` slots and a memory decision rather than a substitution. `deep_causality_uncertain` reaches `mean`, `std_dev` and `MeanAccumulator` and none of these four. Belongs in its own change, against the crate that owns them.
