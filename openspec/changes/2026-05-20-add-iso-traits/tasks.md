...
- [ ] 1.15 **Deviation: pre-existing no_std bug blocks verification.** `cargo build -p deep_causality_num --no-default-features --features libm_math` fails on PRE-EXISTING errors in `src/float_106/ops_arithmetic.rs:255` (`f64::trunc` not found) and `src/algebra/field_real.rs` (multiple `f64::mul_add` not found). These errors exist in the unchanged code; my iso module uses only `core::*` paths and is no_std-clean by inspection. Filing this as a separate finding rather than blocking Stage A.
