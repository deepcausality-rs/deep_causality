...
- [ ] 1.4 Add a `sorry`/obligation CI guard: fail if a `sorry` appears outside the whitelisted `Quantum/*` obligation slots. *(all current Lean files are zero-`sorry`; the guard becomes load-bearing when the QCM obligation layer lands — build it before or with that change)*
