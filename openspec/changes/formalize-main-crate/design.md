...
Source basis: `openspec/notes/causal-algebra/{Formalization.md, Causaloid-Formalization.md, deepcausality-formalization.md}`, `openspec/notes/quantum/{QCM-on-EPP.md, quantum-epp.md}`, and the three `ctx/papers/` quantum-causal-model papers (Lorenz 2022; Lorenz–Barrett 2001.07774; Barrett–Lorenz–Oreshkov 2002.12157). The papers' definitions have been read and verified directly; four findings bind this design: QCM parent sets are labeled and generally overlapping (a mechanism is one channel `ρ_{A|Pa(A)}` over its labeled parent product); commutativity appears only as the checkable `PairwiseCommute` predicate; fan-out duplication is classical-only (no-cloning — the unitary decomposition resolves shared access into direct-sum sectors, 2001.07774 Eq. 14); cyclic admissibility is `ValidProcess`, not an evaluation order. The prerequisite `comonoid-graph-join` change was redesigned around these findings (labeled fan-in over an interpreter-neutral substrate; the earlier commutative-monoid join is retired).
...
**D2 — Reconvergence = labeled fan-in over an interpreter-neutral substrate; comonoid narrowed to the classical fold.** The substrate (delivered by `comonoid-graph-join`) is the labeled structure only: wires keyed by `(parent, child)`, one mechanism per node bound to `Pa(n)`; it fixes no copy, no schedule, no admissibility. The classical evaluator and the QCM assembler are two folds over it. The Markov-category reading survives *scoped*: copy/discard are laws of the classical fold's target (where the diagonal exists), never of the substrate — quantum instantiation replaces copy with commuting access to the shared parent output (`PairwiseCommute`), per the verified papers. `∇_G` as a standalone join operation is dissolved: fan-in is the labeled parent product consumed by the node's own mechanism, and the determinism theorems (`unique_valuation`, `schedule_invariance`) carry no algebraic hypotheses on mechanisms. Classical schedule-invariance and quantum pairwise-commutation are the same theorem shape — "the linearization does not matter" — one proved structurally, one assumed as the Markov predicate. *Retired alternative:* a user-declared commutative-monoid join (erases parent identity; cannot express overlapping parent sets; untypeable on generic `V`).

**D3 — do() = surgery AND handler; one surgery, two folds.** Static graph surgery for the causal hypergraph, built from the substrate primitive defined in `comonoid-graph-join` D10: **wire-key deletion + mechanism pinning**. `do(X = x)` = delete every in-wire key of `X` and pin `X`'s mechanism to the constant `x`; the finer single-edge cut (`delete (P₁, X), keep (P₂, X)`) is expressible because parent contributions are keyed. The *same* surgery is respected by both interpreters — truncated factorization classically, the Lorenz–Tull "opening" quantumly — so intervention is one operation on the substrate, not two frameworks. An alternate `Free::fold` algebra covers dynamic `RelayTo` interventions. Prove `do` functorial w.r.t. encapsulation. Rationale: covers both the static-structure and dynamic-reasoning axes; matches JKZ (surgery) and ChiRho (handler) prior art. Chosen per user decision.
...
**D5 — Acyclicity is a relaxable, freeze-enforceable parameter; admissibility is per-interpretation.** The graph carries acyclicity as a separable constraint. What replaces it when relaxed differs by instantiation: acyclic classical = well-foundedness (the `unique_valuation` induction); cyclic classical = fixpoint semantics (future work, not claimed here); cyclic quantum = the `ValidProcess` predicate — a global operator condition (positivity + normalization; it excludes grandfather paradoxes), **not** an evaluation order or fixpoint. In the acyclic regime the constraint maps to the existing `ultragraph::has_cycle` freeze gate (and constrains do()-surgery to remain acyclic); relaxing it admits cyclic QCMs (quantum switch / indefinite causal order) with the identical labeled factorization/commutativity apparatus. Chosen per user decision.
...
- **[Engine gap — resolved by prerequisite]** The labeled fan-in (wire-slot resolution, per-node join mechanisms, `unique_valuation`/`schedule_invariance` theorems) is delivered by the front-loaded `comonoid-graph-join` change, which lands first. → This change depends on it, so it formalizes real engine behaviour; no witness divergence to manage. If `comonoid-graph-join` slips, this change's graph-reasoning group (§3) and the surgery attachment (§5) are blocked, not its causaloid/context/QCM groups.
