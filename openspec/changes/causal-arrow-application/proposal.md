...
- **APIs:** none changed.
- **Verification:** per-example assertion tests that AD results equal the replaced hand-coded values; integrator rewrites reproduce the prior `Euler` trajectory and improve under `Rk4`; the MMS example verifies the solver against an exact manufactured solution within tolerance.
