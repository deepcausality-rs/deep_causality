...
- [ ] 2.1 Extract the existing Vietoris-Rips lumped-mass Hodge ⋆ construction from `op_triangulate.rs` into a private helper `pub(super) fn build_lumped_mass_hodge_ops<T>(skeletons: &[Skeleton], coords: &[T], dim: usize) -> Result<Vec<CsrMatrix<T>>, TopologyError>`. The existing `triangulate` calls the helper; behaviour is byte-identical (verify with the existing test suite, no diffs in any test output).
...
- [ ] 2.8 Call `build_lumped_mass_hodge_ops` to populate the Hodge ⋆ operators.
