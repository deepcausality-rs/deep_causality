...
- **`ChainComplex` splits in two.** Its six homology items (`num_cells`, `max_dim`,
  ...
  `uniform_lattice_layout`) stay in topology on a new `CellComplex: ChainComplex` supertrait.
...
- **`Gf2Chain` gains complex identity**, so `add`, `intersect` and `inner` stop accepting a chain
  from a different complex that happens to have the same cell count.
...
- `homology-gf2-chain`: the bit-packed mod-2 chain carrying its degree and the identity of its
  complex, with support enumeration, the mod-2 pairing and intersection.
