...
- [ ] 1.1 Write `openspec/notes/homology/reference/reference.py` in the style of
      `openspec/notes/linear/reference/reference.py`: exact arithmetic, no import of this
      workspace, emits constants to paste
- [ ] 1.2 Emit boundary matrices and Betti numbers over ℚ and 𝔽₂ for the minimal triangulations of
      point, interval, `S¹`, `S²`, `T²`, `T³`, cylinder, Möbius band, `RP²` and the Klein bottle
- [ ] 1.3 Cite the published values for each in the docstring — Hatcher, *Algebraic Topology*,
...
- [ ] 1.4 Emit 𝔽₂ kernel and image bases for a range of shapes: square, wide, tall, zero,
      full-rank, and rank-deficient, at sizes crossing the 64-bit word boundary (63, 64, 65, 129)
- [ ] 1.5 Record why `RP²` and the Klein bottle are in the set: every complex the workspace ships
...
- [ ] 2.1 **M** Add `PackedGf2Vector::from_column` and `Gf2Chain::from_column` as signatures with
      `todo!()` bodies and full docs
- [ ] 2.2 **T** Write the suite: for each shape from task 1.4, read every `kernel_basis_gf2` vector
...
- [ ] 2.3 **T** Add the negative test that fixes the orientation: reading the same basis through
      `from_row` gives a vector of the wrong length whenever the matrix is not square
- [ ] 2.4 **V** Confirm no expectation came from a run of `kernel_basis_gf2`; `A · v = 0` is a
...
- [ ] 2.5 **I** Implement `from_column`, reading across the word stride rather than copying a
      contiguous word slice
- [ ] 2.6 Correct the four docstrings that say rows: `packed_gf2_vector/mod.rs:107-108`,
      `gf2_chain/mod.rs:73`, `packed_gf2_vector_tests.rs:177`, `gf2_chain_tests.rs:100`
- [ ] 2.7 Move `widen_to_dense_i64` from `topology/src/types/homology_field/mod.rs` into
      `linear/src/extensions/conversions.rs` as `csr_i8_to_dense_i64`, exported from `lib.rs`
- [ ] 2.8 Bump `deep_causality_linear` to 0.1.2; `cargo test -p deep_causality_linear`, clippy, fmt

## 3. Create the crate skeleton

- [ ] 3.1 `deep_causality_homology/Cargo.toml` at 0.1.0 with description, license, repository,
      `[lints] workspace = true`, and path deps on `deep_causality_linear`,
      `deep_causality_algebra`, `deep_causality_num`
- [ ] 3.2 Add the crate to the workspace `members` list
- [ ] 3.3 `deep_causality_homology/BUILD.bazel` and `tests/BUILD.bazel` with a `rust_test_suite`
      per test directory
- [ ] 3.4 Register in `.github/workflows/formalization.yml` — omission produces a false
      `MISSING Rust witness` on every new theorem id
- [ ] 3.5 Register in `.github/workflows/rust_deps.yml` — omission is silent, cargo-machete simply
      never inspects the crate
- [ ] 3.6 Add the crate index entry and tier block to `AGENTS.md`
- [ ] 3.7 `cargo build -p deep_causality_homology` and `bazel build //deep_causality_homology/...`
      on an empty lib

## 4. Move the chain-complex layer

- [ ] 4.1 **M** Land the full moved API as signatures with `todo!()` bodies: `ChainComplex` with
      ...
      `HomologyError`
- [ ] 4.2 **M** Keep `boundary_matrix` and `coboundary_matrix` returning `Cow<'_, CsrMatrix<i8>>`;
      add no coefficient parameter
- [ ] 4.3 **T** Move the existing tests and rewrite every expectation against task 1.2's values,
      dropping any that was a transcript of a previous run
- [ ] 4.4 **V** Audit the moved suite. Record which tests asserted a quantity only where it
      vanishes, which used a single size, and which compared two results from the same routine
- [ ] 4.5 **V** Add the Euler-characteristic cross-check for every fixture:
...
- [ ] 4.6 **I** Fill the bodies, calling linear's `csr_i8_to_dense_i64` from task 2.7
- [ ] 4.7 Add a probe asserting the crate's dependency set excludes `deep_causality_topology`

## 5. Add the topology seam

- [ ] 5.1 Add `deep_causality_homology` as a path dependency of `deep_causality_topology`
- [ ] 5.2 Define `pub trait CellComplex: ChainComplex` in `topology/src/traits/` with `CellType`,
      `CellIter`, `Metric`, `cells(k)` and `uniform_lattice_layout()`
- [ ] 5.3 Re-export `ChainComplex`, `HomologyField` and `Gf2Chain` from `topology/src/lib.rs`
- [ ] 5.4 Split each of the three implementors — `SimplicialComplex`, `CellComplex<C>`,
      `LatticeComplex` — into a `ChainComplex` impl and a `CellComplex` impl
- [ ] 5.5 Build `deep_causality_cfd` and `deep_causality_physics` with **no** `Cargo.toml` edit and
      **no** `use` edit. A required edit means the re-export in 5.3 is wrong
- [ ] 5.6 Bump `deep_causality_topology` to 0.7.4
...
- [ ] 6.1 **T** Write the table test first: equal degree with unequal length, unequal degree with
      equal length, and both unequal — each refused, each with the same error variant
- [ ] 6.2 **V** Confirm the table covers both halves independently. A test that varies degree and
      length together cannot tell which guard fired
- [ ] 6.3 **I** Replace `same_degree` with `same_group`, checking degree and length together and
      naming `C_k` in the message
- [ ] 6.4 **I** Route `add`, `intersect` and `inner` through it, so a length mismatch stops
      surfacing as a `PackedGf2Vector` error from a layer below
- [ ] 6.5 Record in the type docstring why there is no complex handle — every operation it has
      belongs to `C_k`, and the complex enters with `∂`

## 7. State and test the differential law

- [ ] 7.1 **M** State `∂ₖ ∘ ∂ₖ₊₁ = 0` in the `ChainComplex` docstring and in `Cell`'s, since
      `CellComplex<C>` derives every operator from `Cell::boundary()`
- [ ] 7.2 **T** Assert it in the conformance harness for every implementor at every grade, widening
      coefficients past `i8` before multiplying so the assertion does not run on wrapping arithmetic
      in release
- [ ] 7.3 **T** Add `CellComplex` to the conformance harness with a concrete `Cell` fixture; it is
      absent today
- [ ] 7.4 **V** Verify the harness discriminates: a deliberately malformed complex — one incidence
      sign flipped — must fail it. A harness that passes on the malformed complex is measuring
      nothing
- [ ] 7.5 **V** Confirm the assertion runs at grades where `∂` is non-zero, not only at the
      degenerate ends where the product is empty

## 8. Fix the degenerate-grade shape

- [ ] 8.1 **T** Write the shape assertions first: `∂₀` has shape `(0, num_cells(0))` and `∂_{max+1}`
      has shape `(num_cells(max), 0)`, for all three implementors. They fail against
      `CsrMatrix::new()`
- [ ] 8.2 **T** Assert every Betti number is unchanged at every grade of every fixture over both
      fields, against task 1.2's values
- [ ] 8.3 **I** Change all three implementors
- [ ] 8.4 Extend `assert_shape_invariant` from `1..=max_d` to cover both ends
- [ ] 8.5 **R** Repair `lattice_complex_test.rs:183-193`, which asserts `cols(∂₂) == rows(∂₁)` —
...
- [ ] 9.1 Add `lean/DeepCausalityFormal/Homology/ChainCondition.lean` proving
...
- [ ] 9.2 Prove `homology.chain.betti_from_dd_zero`: `gf2_betti_from_ranks` restated with
      `∂ₖ ⬝ ∂ₖ₊₁ = 0` in place of the inclusion. Zero `sorry`; `#print axioms` reports only
      `propext`, `Classical.choice`, `Quot.sound`
- [ ] 9.3 Add `"Homology"` to `_NAMESPACES` in `lean/BUILD.bazel`, anchored on
      `DeepCausalityFormal/Homology/ChainCondition.lean`. Without it the file is never type-checked
- [ ] 9.4 Add any new Mathlib import to `cache_roots` in `MODULE.bazel` — `Matrix.mulVecLin_mul` is
...
- [ ] 9.5 Write the two Rust witnesses in
...
- [ ] 9.6 Add two rows to `lean/THEOREM_MAP.md` in the existing column format
- [ ] 9.7 Write `deep_causality_homology/LEAN_HOMOLOGY.md` following `LEAN_LINEAR.md`: what is
      formalized and why it is load-bearing, the L1/L2/bridge split, model fidelity, and what is
      left unformalized with the reason
- [ ] 9.8 `bazel test //lean:Homology` and `bazel test //lean:proofs`

## 10. Verify and record

- [ ] 10.1 `bazel test //...` — every test passes, and the count is at least the pre-change count
- [ ] 10.2 Run `cargo mutants` over `deep_causality_homology/src`. A survivor is a decision the
...
- [ ] 10.3 `cargo clippy --workspace --all-targets -- -D warnings`, fixing by rewriting rather than
      by `#[allow]`
- [ ] 10.4 `cargo fmt --all -- --check`
- [ ] 10.5 Generate the SBOM pair for the new crate
- [ ] 10.6 Record in `openspec/notes/quantum/qcl-gaps.md` that G-04 and G-08 now have a home, and
