...
2. ~~**Should the regression-test fixtures cover all three classified error categories, or only the two that source-changes affect ((a) duplicate-point and (b) volume-below-tolerance)?**~~ **Resolved at H0-1.3:** branch (c) (singular Gram matrix) is sub-classified under (b) and produces the same `"top-dimensional simplex...below tolerance"` error message regardless of which detection path fires. The H2 regression suite therefore covers two classified categories: duplicate-point (one test) and volume-degenerate (two tests covering the threshold-compare path via collinear input and the Gram-singular path via coplanar 4-clique in 3D). A dedicated third test for the Gram-singular path is unnecessary because the unified error message means an end-to-end caller cannot distinguish the two detection paths.
