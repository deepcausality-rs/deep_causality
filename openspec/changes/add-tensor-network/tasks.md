...
- [x] 6.2 Instantiate and test the stack at `Complex<f64>`: **state trains** (TT-SVD round-trip, conjugated inner with real `⟨x|x⟩`, real norm, complex add/scale, lossless round + isometric canonical cores) and **MPO operators** (operator TT-SVD round-trip, `apply` = MPO·MPS vs dense matvec, `compose` equivalence). New tests: `op_tensor_complex_tests.rs`, `causal_tensor_train_complex_tests.rs`, `causal_tensor_train_operator_complex_tests.rs`. **Still real/dual-only (deferred):** `cross`, `apply_nonlinear`, and the solvers (`fit`/`linear`/`eigen`/`tdvp`) — they need ordered pivoting/epsilon comparisons; not required by the complex acceptance (unitarity/inner/round-trips).
