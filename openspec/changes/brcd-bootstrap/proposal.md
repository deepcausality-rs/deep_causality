...
- Implement only the `local_score_BIC_from_cov` score (the single score the real-world runs use): `n·ln(conditional_variance) + ln(n)·|PA|·λ`, built on the existing `deep_causality_tensor` `sample_covariance` + `conditional_variance` (ridge Schur complement). The RKHS and BDeu scores are **out of scope**.
...
- **New code:** a BOSS module under `deep_causality_algorithms::causal_discovery` (score, GST, order search, DAG→CPDAG wrapper) and the `cpdag = None` branch wiring in `brcd_algo`.
