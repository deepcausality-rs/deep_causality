...
**D1. Module placement.** A new `deep_causality_algorithms::causal_discovery::boss` module, sibling to `surd` and `brcd`, with submodules `boss_score`, `boss_gst`, `boss_search`, and `boss_cpdag`. The public entry point `boss_learn(data: &CausalTensor<T>, config) -> Result<MixedGraph<()>, BrcdError>` returns the learned CPDAG. Errors reuse the existing `BrcdError`/`BrcdErrorEnum` (extended with a `StructureLearning`-style variant only if a genuinely new failure case appears).

**D2. Score = BIC-from-cov only, over existing tensor primitives.** Compute the `p×p` sample covariance **once** with `CausalTensorStatsExt::sample_covariance`. A family `(i, PA)` scores as:
- no parents: `n · ln(cov[i,i])`;
- with parents: `n · ln(conditional_variance(i, PA, ε)) + ln(n) · |PA| · λ`,
where `conditional_variance` is the ridge Schur complement `Σ_yy − Σ_yP (Σ_PP + εI)⁻¹ Σ_Py` already in `deep_causality_tensor`. The reference uses `ε = 1e-6` and `λ = 2`; these are config fields. This is the only score implemented.
