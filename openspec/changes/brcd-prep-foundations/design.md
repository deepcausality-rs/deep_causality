...
**D2. The graph *type* is `deep_causality_topology::MixedGraph`; only the causal *operations* live in `deep_causality_algorithms`.** *(Revised — the `mixed-graph` change landed and is archived.)*
The CPDAG/PDAG representation is no longer built here. It is the typed-endpoint `MixedGraph` delivered by the `mixed-graph` change in `deep_causality_topology`, which already provides the directed-arc projection (`parents`/`children`), undirected neighbors, per-kind edge enumeration, the endpoint invariant, and a self-contained topological sort / cycle check. This change therefore adds only the causal *operations* — Meek orientation, the unshielded-collider validity check, and MEC sizing — as modules under `deep_causality_algorithms::causal_discovery` (e.g. `meek`, `validity`, `mec`) beside `surd`, consuming `MixedGraph`. *Alternatives considered:* (a) define a PDAG type in the algorithms crate — rejected, it would duplicate `MixedGraph`; (b) host the operations in `deep_causality_topology` too — rejected, Meek/MEC are causal-discovery algorithms and belong in the algorithms crate. **Open dependency question (see Open Questions):** `deep_causality_algorithms` does not currently depend on `deep_causality_topology`, and `topology` is a heavyweight crate; consuming `MixedGraph` adds that dependency.
...
- **`deep_causality_algorithms` → `deep_causality_topology` dependency (D2).** Consuming `MixedGraph` requires the algorithms crate to depend on the topology crate, which is heavyweight (Hodge, manifolds, gauge, …). Decide before implementing Tier B: (a) accept the dependency; (b) host the Meek/MEC/validity operations in `deep_causality_topology` beside `MixedGraph`; or (c) relocate `MixedGraph` to a lighter shared crate. Default: (a), accept the dependency, since the operations are causal-discovery algorithms and belong in the algorithms crate.
