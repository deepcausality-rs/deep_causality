...
- **`CurvatureTensor<T, ...>` → `CurvatureTensor<R: RealField, ...>`.** Drops `From<f64> + Into<f64>` from every impl block. Replaces every `<T as From<f64>>::from(...)` site with a `RealField`-native expression or a `R::from_f64(...)` call against the constructor added to `RealField` (see Decision 2 in `design.md`).
...
- **`deep_causality_num::RealField` gets one additive method: `fn from_f64(value: f64) -> Self`.** This is the only cross-crate change in R0. The method is necessary because some numerical work needs to materialize constants like `1e-12` (epsilon tolerances) or `2.0` (binary half) that aren't expressible via `R::one() / R::epsilon() / R::pi() / R::e()` alone. `f32` and `f64` impls are one-line casts; future impls (`f128`, dual numbers, etc.) implement the trivial conversion. **No default impl.** Every implementor provides the method explicitly. See `design.md` for the alternatives considered (`R::from_i64`, `R::two()`, `R::half()`, etc.).
...
- **`RealField::from_f64` additive method.** The only trait surface change in R0. `f32`, `f64` impls are one-line casts. Documented in detail in `design.md` Decision 2.
