...
use deep_causality_linear::{
    CsrMatrix, csr_i8_to_dense_i64, csr_to_packed_gf2_mod2, rank_exact, rank_gf2,
};
...
impl HomologyField {
    ...
    /// # Errors
    ///
    /// [`HomologyErrorEnum::LinearAlgebraError`](crate::HomologyErrorEnum::LinearAlgebraError) if
    ...
    pub fn rank_of(self, matrix: &CsrMatrix<i8>) -> Result<usize, HomologyError> {
        ...
        match self {
            ...
            Self::Rational => {
                let widened = csr_i8_to_dense_i64(matrix);
                rank_exact(&widened).map_err(HomologyError::from)
            }
            Self::Gf2 => {
                let packed = csr_to_packed_gf2_mod2::<u64>(matrix);
                rank_gf2(&packed).map_err(HomologyError::from)
            }
        }
    }
}
