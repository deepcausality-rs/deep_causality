...
fn test_index_out_of_bounds_names_the_position_and_the_shape() {
    let e = LinearError::IndexOutOfBounds {
        index: (3, 1),
        shape: (2, 2),
    };
    let s = format!("{e}");
...
fn test_shape_mismatch_names_both_shapes() {
    let e = LinearError::ShapeMismatch {
        left: (2, 3),
        right: (4, 5),
    };
    let s = format!("{e}");
...
fn test_inner_dimension_mismatch_names_both_dimensions() {
    let e = LinearError::InnerDimensionMismatch {
        left_cols: 3,
        right_rows: 7,
    };
    let s = format!("{e}");
...
fn test_length_mismatch_names_expected_and_found() {
    let e = LinearError::LengthMismatch {
        expected: 4,
        found: 9,
    };
    let s = format!("{e}");
...
fn test_not_binary_names_the_position_so_the_caller_need_not_rescan() {
    let e = LinearError::NotBinary { at: (1, 3) };
    let s = format!("{e}");
...
fn test_errors_compare_and_clone() {
    let a = LinearError::Singular { at_column: 1 };
    assert_eq!(a.clone(), a);
    assert_ne!(a, LinearError::Singular { at_column: 2 });
