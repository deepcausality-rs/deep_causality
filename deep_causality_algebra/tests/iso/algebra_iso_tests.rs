...
//! Tests for `deep_causality_algebra::iso::AlgebraIso<T, R>` and the helper
//! `assert_algebra_iso_from_law`.
//!
//! Uses the shared `FloatWrap(f64)` newtype from `deep_causality_algebra::utils_tests::utils_iso_tests`. `FloatWrap`
//! is an `Algebra<f64>` (Module<f64> + Mul<Output = Self> + One + Distributive).
//! The identity iso preserves scalar multiplication trivially.

use deep_causality_algebra::iso::test_support::assert_algebra_iso_from_law;
