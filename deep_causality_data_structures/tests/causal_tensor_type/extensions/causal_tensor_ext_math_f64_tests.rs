...
#[test]
fn test_log_nat() {
    let data = vec![1.0f64, std::f64::consts::E, 10.0f64];
    let tensor = CausalTensor::new(data, vec![3]).unwrap();
    let log_tensor = tensor.log_nat().unwrap();

    let expected = [0.0f64, 1.0f64, std::f64::consts::LN_10];
    log_tensor
        .as_slice()
        .iter()
        .zip(expected.iter())
...
fn test_log_nat_special_values() {
    let data = vec![0.0f64, -1.0f64];
    let tensor = CausalTensor::new(data, vec![2]).unwrap();
    let log_tensor = tensor.log_nat().unwrap();
    let slice: &[f64] = log_tensor.as_slice();
...
#[test]
fn test_log2() {
    let data = vec![1.0f64, 2.0f64, 8.0f64, 16.0f64];
    let tensor = CausalTensor::new(data, vec![4]).unwrap();
    let log_tensor = tensor.log2().unwrap();

    let expected = [0.0f64, 1.0f64, 3.0f64, 4.0f64];
    log_tensor
        .as_slice()
        .iter()
        .zip(expected.iter())
...
#[test]
fn test_log10() {
    let data = vec![1.0f64, 10.0f64, 100.0f64];
    let tensor = CausalTensor::new(data, vec![3]).unwrap();
    let log_tensor = tensor.log10().unwrap();

    let expected = [0.0f64, 1.0f64, 2.0f64];
    log_tensor
        .as_slice()
        .iter()
        .zip(expected.iter())
...
#[test]
fn test_surd_log2() {
    let data = vec![1.0f64, 2.0f64, 0.0f64, 16.0f64];
    let tensor = CausalTensor::new(data, vec![4]).unwrap();
    let log_tensor = tensor.surd_log2().unwrap();

    let expected = [0.0f64, 1.0f64, 0.0f64, 4.0f64];
    log_tensor
        .as_slice()
        .iter()
        .zip(expected.iter())
...
fn test_surd_log2_special_values() {
    ...
    assert_eq!(slice[0], 0.0f64); // surd_log2(0) = 0
...
fn test_safe_div_normal() {
    let a = CausalTensor::new(vec![4.0f64, 9.0f64, 0.0f64], vec![3]).unwrap();
    let b = CausalTensor::new(vec![2.0f64, 3.0f64, 0.0f64], vec![3]).unwrap();
    let result = a.safe_div(&b).unwrap();
    assert_eq!(result.as_slice(), &[2.0f64, 3.0f64, 0.0f64]);
}

#[test]
fn test_safe_div_broadcast() {
    let a = CausalTensor::new(vec![4.0f64, 8.0f64, 12.0f64], vec![3]).unwrap();
    let b_scalar = CausalTensor::new(vec![2.0f64], vec![1]).unwrap();
    let result = a.safe_div(&b_scalar).unwrap();
    assert_eq!(result.as_slice(), &[2.0f64, 4.0f64, 6.0f64]);
}

#[test]
fn test_safe_div_zero_by_zero() {
    let a = CausalTensor::new(vec![0.0f64], vec![1]).unwrap();
    let b = CausalTensor::new(vec![0.0f64], vec![1]).unwrap();
    let result = a.safe_div(&b).unwrap();
    assert_eq!(result.as_slice(), &[0.0f64]);
}

#[test]
fn test_safe_div_by_zero_error() {
    let a = CausalTensor::new(vec![1.0f64], vec![1]).unwrap();
    let b = CausalTensor::new(vec![0.0f64], vec![1]).unwrap();
