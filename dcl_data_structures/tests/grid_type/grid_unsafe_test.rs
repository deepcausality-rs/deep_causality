...
mod tests {
    ...
    use dcl_data_structures::prelude::{Grid, PointIndex};
    ...
    fn test_get_default_value() {
        ...
        let point = PointIndex { x: 0, y: 2, z: 0, t: 0 };
        let value = grid.get(point);
        assert_eq!(value, 42u32);

        // Now test the default value path by setting initialized to false
        unsafe {
            let grid_ptr = &grid as *const Grid<[u32; SIZE_1D], u32> as *mut Grid<[u32; SIZE_1D], u32>;
            let storage_offset = 0;
            let storage_size = std::mem::size_of::<[u32; SIZE_1D]>();
            let initialized_ptr = (grid_ptr as *mut u8).add(storage_offset + storage_size) as *mut AtomicBool;
    ...
    fn test_set_and_get_2d() {
        ...
        let point = PointIndex { x: 1, y: 2, z: 0, t: 0 };
        let value = 42u32;
    ...
    fn test_set_and_get_3d() {
        ...
        let point = PointIndex { x: 1, y: 1, z: 1, t: 0 };
        let value = 42u32;
    ...
    fn test_set_and_get_4d() {
        ...
        let point = PointIndex { x: 1, y: 1, z: 1, t: 1 };
        let value = 42u32;
    ...
    fn test_multiple_set_and_get() {
        ...
        let points = [
            PointIndex { x: 0, y: 0, z: 0, t: 0 },
            PointIndex { x: 1, y: 1, z: 0, t: 0 },
            PointIndex { x: 2, y: 2, z: 0, t: 0 },
        ];
    ...
    fn test_out_of_bounds_2d() {
        ...
        let point = PointIndex { x: SIZE_2D, y: SIZE_2D, z: 0, t: 0 };
        grid.set(point, 42);
    }
    ...
    fn test_1d_grid() {
        ...
        unsafe {
            let grid_ptr = &grid as *const Grid<[i32; SIZE_1D], i32> as *mut Grid<[i32; SIZE_1D], i32>;
            let storage_offset = 0;
