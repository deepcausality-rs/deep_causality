...
fn unit_constructor_marks_unit_edge() {
    let g = CubicalReggeGeometry::<3>::unit();
...
fn uniform_constructor_is_isotropic_but_not_unit_edge() {
    let g = CubicalReggeGeometry::<3>::uniform(0.5);
    assert!(!g.is_unit_edge());
    assert!(g.is_isotropic());
    assert!(g.is_axis_aligned());
}
...
fn per_axis_constructor_is_axis_aligned_but_not_isotropic() {
    let g = CubicalReggeGeometry::<3>::per_axis([1.0, 2.0, 3.0]);
    assert!(!g.is_unit_edge());
    assert!(!g.is_isotropic());
    assert!(g.is_axis_aligned());
}
...
fn per_edge_constructor_is_general() {
    let g = CubicalReggeGeometry::<2>::from_edge_lengths(vec![1.0, 1.5, 2.0, 2.5]);
    assert!(!g.is_unit_edge());
    assert!(!g.is_isotropic());
    assert!(!g.is_axis_aligned());
}
...
fn uniform_length_returns_value_for_uniform() {
    let g = CubicalReggeGeometry::<3>::uniform(0.25);
...
fn axis_lengths_broadcasts_uniform() {
    let g = CubicalReggeGeometry::<3>::uniform(2.5);
...
fn axis_lengths_is_none_for_per_edge() {
    let g = CubicalReggeGeometry::<2>::from_edge_lengths(vec![1.0, 2.0]);
...
fn edge_length_at_returns_value_for_uniform() {
    let g = CubicalReggeGeometry::<3>::uniform(0.75);
...
fn edge_length_at_indexes_per_edge() {
    let g = CubicalReggeGeometry::<2>::from_edge_lengths(vec![1.0, 1.5, 2.0]);
    assert_eq!(g.edge_length_at(0), Some(1.0));
...
fn with_timelike_axes_attaches_pattern() {
    let g = CubicalReggeGeometry::<4>::unit().with_timelike_axes([false, false, false, true]);
...
fn all_spacelike_axes_is_not_lorentzian() {
    let g = CubicalReggeGeometry::<3>::unit().with_timelike_axes([false, false, false]);
    assert!(!g.is_lorentzian());
}
...
fn equality_distinguishes_representations() {
    let unit_a = CubicalReggeGeometry::<2>::unit();
    let unit_b = CubicalReggeGeometry::<2>::unit();
    assert_eq!(unit_a, unit_b);

    let uniform_one = CubicalReggeGeometry::<2>::uniform(1.0);
