...
fn test_witness_new_and_display() {
    let witness = LatticeGaugeFieldWitness::<TestGroup, D>::new();
...
fn test_pure() {
    let beta = 5.0;
    let field = LatticeGaugeFieldWitness::<TestGroup, D>::pure(beta);
...
#[test]
fn test_functor() {
    let field = LatticeGaugeFieldWitness::<TestGroup, D>::pure(2.0);
    // Map beta: 2.0 -> 4.0
...
#[test]
fn test_applicative() {
    let func_field = LatticeGaugeFieldWitness::<TestGroup, D>::pure(|x: f64| x + 10.0);
    let val_field = LatticeGaugeFieldWitness::<TestGroup, D>::pure(5.0);
...
#[test]
fn test_monad() {
    let field = LatticeGaugeFieldWitness::<TestGroup, D>::pure(3.0);
    let result = LatticeGaugeFieldWitness::bind(field, |x| {
        LatticeGaugeFieldWitness::<TestGroup, D>::pure(x * x)
    });
...
fn test_map_field_full() {
    ...
    let field = LatticeGaugeField::<TestGroup, D, f64>::identity(lattice, 1.0);
...
fn test_scale_field() {
    ...
    let scaled = LatticeGaugeFieldWitness::scale_field(field, 0.5);
...
fn test_zip_with_success() {
    let shape = [2, 2];
    let lattice = Arc::new(Lattice::new(shape, [true, true]));
    let field_a = LatticeGaugeField::<TestGroup, D, f64>::identity(lattice.clone(), 1.0);
    let field_b = LatticeGaugeField::<TestGroup, D, f64>::identity(lattice, 2.0);
...
fn test_zip_with_lattice_mismatch() {
    ...
    let field_a = LatticeGaugeField::<TestGroup, D, f64>::identity(lattice1, 1.0);
    let field_b = LatticeGaugeField::<TestGroup, D, f64>::identity(lattice2, 1.0);
...
fn test_zip_with_missing_link() {
    ...
    let field_b = LatticeGaugeField::<TestGroup, D, f64>::from_links_unchecked(
        lattice,
...
fn test_identity_field_wrapper() {
    ...
    let shape = [2, 2];
    let lattice = Arc::new(Lattice::new(shape, [true, true]));
    let field = LatticeGaugeFieldWitness::<TestGroup, D>::identity_field(lattice, 1.0).unwrap();
