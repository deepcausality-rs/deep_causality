...
impl<T: TensorData + RealField + Sum + Neg<Output = T>> InternalCpuTensor<T> {
    ...
    pub(in crate::types::cpu_tensor) fn svd_impl(
        &self,
        ...
        let u = InternalCpuTensor::new(u_data, vec![m, k])?;
        let s = InternalCpuTensor::new(s_data, vec![k])?;
        let vt = InternalCpuTensor::new(vt_data, vec![k, n])?;
