...
impl<T: TensorData + RealField + Neg<Output = T>> InternalCpuTensor<T> {
    ...
    pub(in crate::types::cpu_tensor) fn qr_impl(&self) -> Result<(Self, Self), CausalTensorError>
    where
        T: Sum,
    {
        if self.ndim() != 2 {
        ...
        let q = InternalCpuTensor::new(q_data, vec![m, m])?;
        let r = InternalCpuTensor::new(r_data, vec![m, n])?;
