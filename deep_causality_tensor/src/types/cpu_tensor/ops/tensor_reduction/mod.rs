...
{
    pub(in crate::types::cpu_tensor) fn norm_l2_impl(&self) -> T
    where
    ...
    pub(in crate::types::cpu_tensor) fn norm_sq_impl(&self) -> T
    where
    ...
    pub(in crate::types::cpu_tensor) fn sum_axes_impl(
        &self,
        axes: &[usize],
        ...
        T: Add<T, Output = T>,
    {
        ...
        if self.data.is_empty() {
            ...
            return Ok(InternalCpuTensor::full(&result_shape, T::default()));
        }
        ...
        let mut result_tensor = InternalCpuTensor::full(&result_shape, T::default());
    ...
    pub(in crate::types::cpu_tensor) fn mean_axes_impl(
        &self,
        axes: &[usize],
    ) -> Result<Self, CausalTensorError>
    where
    ...
    pub(in crate::types::cpu_tensor) fn arg_sort_impl(
        &self,
    ...
    pub fn max_axes_impl(&self, axes: &[usize]) -> Result<Self, CausalTensorError>
    where
        T: Copy,
    {
        // Check for invalid axes first.
        ...
        // Special case: max of all elements to a scalar tensor.
        if axes.is_empty() {
            if self.data.is_empty() {
                return Err(CausalTensorError::InvalidOperation); // Cannot max empty
            }
            ...
            return InternalCpuTensor::new(vec![*max_val], vec![]);
        }
        ...
        // Re-do implementation to follow sum_axes_impl structure more closely.
        let mut result_tensor = InternalCpuTensor::full(&result_shape, T::default());
        let mut init_mask = vec![false; result_len];

        let mut current_index = vec![0; self.ndim()];
        for i in 0..self.len() {
            let value = self.data[i];

            // Construct result index
            let mut result_idx_vec = Vec::new();
            for (axis, &val) in current_index.iter().enumerate() {
                if !axes.contains(&axis) {
                    result_idx_vec.push(val);
                }
            }

            let dest_flat = result_tensor.get_flat_index(&result_idx_vec).unwrap();

            if !init_mask[dest_flat] {
                result_tensor.data[dest_flat] = value;
                init_mask[dest_flat] = true;
            } else if value > result_tensor.data[dest_flat] {
                result_tensor.data[dest_flat] = value;
            }
