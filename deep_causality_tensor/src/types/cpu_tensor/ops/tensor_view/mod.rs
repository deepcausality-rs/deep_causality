    ...
    T: Clone,
{
    ...
    pub(in crate::types::cpu_tensor) fn shifted_view_impl(&self, flat_index: usize) -> Self
    where
        T: Clone,
    {
    ...
    /// # Returns
    ///
    /// A new `InternalCpuTensor` with shape determined by the range sizes.
    ///
    /// # Errors
    ///
    /// - `DimensionMismatch` if `ranges.len() != self.ndim()`
    ...
    pub fn range_slice_impl(
        ...
        if new_len == 0 {
            ...
            return InternalCpuTensor::new(vec![], new_shape);
        }
