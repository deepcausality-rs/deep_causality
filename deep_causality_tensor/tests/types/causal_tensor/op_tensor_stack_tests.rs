...
fn test_stack_empty_slice() {
    ...
    let result = CausalTensor::stack(&tensors, 0);
...
fn test_stack_axis_out_of_bounds() {
    ...
    let result = CausalTensor::stack(&tensors, 2);
    assert!(result.is_err());
...
fn test_stack_scalars() {
    ...
    // Stack along axis 0
    let result = CausalTensor::stack(&tensors, 0).unwrap();
...
fn test_stack_vectors_axis_1() {
    ...
    let result = CausalTensor::stack(&tensors, 1).unwrap();
    assert_eq!(result.shape(), &[2, 2]);
...
fn test_stack_matrices_axis_2() {
    ...
    let result = CausalTensor::stack(&tensors, 2).unwrap();
    assert_eq!(result.shape(), &[2, 2, 2]);
