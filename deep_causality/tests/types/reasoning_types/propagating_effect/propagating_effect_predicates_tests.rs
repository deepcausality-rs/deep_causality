...
#[test]
fn test_is_ok() {
    let effect_ok = PropagatingEffect::from_boolean(true);
...
#[test]
fn test_has_log() {
    let effect_no_log = PropagatingEffect::from_boolean(true);
    assert!(!effect_no_log.has_log());

    let mut effect_with_log = PropagatingEffect::from_boolean(true);
