...
fn test_graph_evaluate_success() {
    ...
    let effect = PropagatingEffect::from_value(0.99); // A value that will activate all nodes
...
fn test_graph_evaluate_error_root_not_found() {
    ...
    // Change generic to bool, bool to match inherent impl requirements (V -> V)
    let mut g = CausaloidGraph::<BaseCausaloid<bool, bool>>::new(0);
