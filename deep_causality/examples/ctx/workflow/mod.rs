...
use std::error::Error;
use deep_causality::prelude::{ContextGraph, ContextKind, ContextMatrixGraph, Spaceoid, SpaceTempoid, Tempoid, TimeScale};
...
pub fn build_time_data_context(
    ...
    -> Result<ContextGraph<Dataoid, Spaceoid, Tempoid, SpaceTempoid>, Box<dyn Error>>
{
    let kind = ContextKind::TimeData;
    let graph = match generate_time_data_context_graph(data, max_time_scale) {
        Ok(g) => g,
        Err(e) => return Err(e),
    };

    Ok(ContextGraph::new(id, name, kind, graph))
}
