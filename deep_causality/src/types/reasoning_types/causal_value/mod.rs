...
/// Represents the payload of a causal effect.
///
/// This enum encapsulates various types of data that can be propagated
...
pub enum CausalValue {
    ...
    Map(HashMap<IdentificationValue, Box<CausalValue>>),
    /// A graph of effects, for passing complex relational data.
    Graph(Arc<UltraGraph<CausalValue>>),
