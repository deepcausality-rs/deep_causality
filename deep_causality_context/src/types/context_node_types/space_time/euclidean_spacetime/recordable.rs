...
use crate::{EuclideanSpacetime, SpaceTemporal, Temporal};
    ...
    for EuclideanSpacetime<R>
{
    ...
    fn from_record(id: ContextoidId, record: SpaceTimeRecord) -> Result<Self, ProjectionError> {
        match record {
            SpaceTimeRecord::Euclidean { x, y, z, t, scale } => Ok(EuclideanSpacetime::new(
                id,
                lift(id, x)?,
                lift(id, y)?,
                lift(id, z)?,
