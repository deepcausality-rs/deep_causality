...
//! `Recordable<SpaceTimeRecord>` for EuclideanSpacetime. Expected records are the constructor literals under
...
use deep_causality_context::{EuclideanSpacetime, TimeScale};
use deep_causality_context_store::{ProjectionError, Recordable, SpaceTimeRecord};
use deep_causality_num::{BFloat16, Float106};

#[test]
fn test_round_trip() {
    let node = EuclideanSpacetime::new(3, 1.5, 2.5, 3.5, 4.5, TimeScale::Millisecond);
    ...
    assert_eq!(EuclideanSpacetime::from_record(3, record), Ok(node));
}
...
fn test_every_other_variant_is_refused() {
    ...
    for (found, record) in others {
        assert_eq!(
            EuclideanSpacetime::<f64>::from_record(9, record),
...
fn test_zero_and_negative_round_trip() {
    for node in [
        EuclideanSpacetime::new(1, 0.0, 0.0, 0.0, 0.0, TimeScale::NoScale),
        EuclideanSpacetime::new(1, -1.0, -2.0, -3.0, -4.0, TimeScale::Year),
    ] {
        assert_eq!(
            EuclideanSpacetime::from_record(1, node.to_record().unwrap()),
            Ok(node)
        );
    }
}
...
fn test_non_finite_round_trips() {
    let node = EuclideanSpacetime::new(1, 0.0, 0.0, 0.0, f64::INFINITY, TimeScale::Second);
    assert_eq!(
        EuclideanSpacetime::from_record(1, node.to_record().unwrap()),
        Ok(node)
    );
...
fn test_precision_is_spent_at_the_bound() {
    ...
    let wide = EuclideanSpacetime::new(2, f(1.0), f(2.0), f(3.0), f(4.0), TimeScale::Second);
    let restored =
        EuclideanSpacetime::<Float106>::from_record(2, wide.to_record().unwrap()).unwrap();
    let e = |v: f64| Float106::new(v, 0.0);
    assert_eq!(
        restored,
        EuclideanSpacetime::new(2, e(1.0), e(2.0), e(3.0), e(4.0), TimeScale::Second)
    );
    let b = |v: f64| BFloat16::from(v);
    let narrow = EuclideanSpacetime::new(2, b(1.5), b(2.5), b(3.5), b(4.5), TimeScale::Second);
    assert_eq!(
        EuclideanSpacetime::<BFloat16>::from_record(2, narrow.to_record().unwrap()),
        Ok(narrow)
    );
}
