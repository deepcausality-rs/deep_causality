...
//! `Recordable<SpaceRecord>` for NedSpace. Expected records are the constructor literals under their
...
fn test_round_trip() {
    ...
    assert_eq!(NedSpace::from_record(3, record), Ok(node));
}
...
fn test_every_other_variant_is_refused() {
    ...
    for (found, record) in others {
        assert_eq!(
            NedSpace::<f64>::from_record(9, record),
            Err(ProjectionError::WrongVariant(9, "Ned", found))
        );
    }
}
...
fn test_zero_and_negative_round_trip() {
    for node in [
        NedSpace::new(1, 0.0, 0.0, 0.0),
        NedSpace::new(1, -1.0, -2.0, -0.5),
    ] {
        assert_eq!(
            NedSpace::from_record(1, node.to_record().unwrap()),
            Ok(node)
        );
    }
}
...
fn test_non_finite_round_trips() {
    let node = NedSpace::new(1, 0.0, f64::NAN, 0.0);
    let restored = NedSpace::<f64>::from_record(1, node.to_record().unwrap()).unwrap();
    assert_ne!(restored, node);
    assert_eq!(restored.to_record().unwrap().kind_name(), "Ned");
}
...
fn test_precision_is_spent_at_the_bound() {
    ...
    let wide = NedSpace::new(
        2,
        Float106::new(1.0, low),
        Float106::new(2.0, low),
        Float106::new(3.0, low),
    );
    let restored = NedSpace::<Float106>::from_record(2, wide.to_record().unwrap()).unwrap();
    assert_eq!(
        restored,
        NedSpace::new(
            2,
    ...
    assert_eq!(
        NedSpace::<BFloat16>::from_record(2, narrow.to_record().unwrap()),
        Ok(narrow)
    );
}
