...
#[test]
fn test_standard_uniform_f32_sample_range() {
    let mut rng = Xoshiro256::new();
    let dist = StandardUniform;

    for _ in 0..1000 {
        let val: f32 = dist.sample(&mut rng);
        assert!(val >= 0.0 && val < 1.0, "Value {} out of range [0.0, 1.0)", val);
    }
}

#[test]
fn test_standard_uniform_f32_sample_randomness() {
    let mut rng = Xoshiro256::new();
    let dist = StandardUniform;
    ...
    assert!(values.iter().any(|&x: &f32| x != values[0]), "Not all values are the same");
    assert!(values.windows(2).any(|w| w[0] != w[1]), "Values are not sequential");
}

#[test]
fn test_open_closed_01_f32_sample_range() {
    let mut rng = Xoshiro256::new();
...
#[test]
fn test_open_closed_01_f32_sample_randomness() {
    let mut rng = Xoshiro256::new();
    let dist = OpenClosed01;
...
#[test]
fn test_open01_f32_sample_range() {
    let mut rng = Xoshiro256::new();
    let dist = Open01;
...
#[test]
fn test_open01_f32_sample_randomness() {
    let mut rng = Xoshiro256::new();
    let dist = Open01;
