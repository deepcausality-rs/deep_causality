...
//! `finalize` is the shared terminal: it packages whatever `CdlDiscoveryOutcome`
//! the analyze step produced into a `CdlReport`.
...
#[test]
fn test_finalize_packages_report() {
    let file = write_csv(
        "s1,s2,s3,target\n1.0,2.0,3.0,1.5\n2.0,4.1,6.0,3.6\n3.0,6.2,9.0,5.4\n4.0,8.1,12.0,7.6",
    );
    ...
    let report_effect = CdlBuilder::build_surd(&cfg)
        .surd_load_input()
