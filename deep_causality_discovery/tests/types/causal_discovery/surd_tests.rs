...
fn test_surd_causal_discovery_success() {
    let data = vec![
        ...
        Some(0.0),
        Some(0.2), // P(T=0, S1=1, S2=0), P(T=0, S1=1, S2=1)
        Some(0.3),
        Some(0.0), // P(T=1, S1=0, S2=0), P(T=1, S1=0, S2=1)
        Some(0.1),
    ...
    // With dummy surd_states_cdl, these will be empty/zero
...
fn test_surd_causal_discovery_error_invalid_parameter() {
    ...
    if let Err(CausalDiscoveryError::TensorError(e)) = result {
        ...
        panic!(
            "Expected CausalDiscoveryError::TensorError(InvalidParameter), got {:?}",
            result
        );
    }
}
