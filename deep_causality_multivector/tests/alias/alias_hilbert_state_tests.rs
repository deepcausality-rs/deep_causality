...
fn test_new_spin10_success() {
    ...
    let state = HilbertState::<f64>::new_spin10(data.clone()).unwrap();
...
fn test_new_spin10_error_data_length_mismatch() {
    ...
    let err = HilbertState::<f64>::new_spin10(data).unwrap_err();
    assert_eq!(
        format!("{}", err),
...
fn test_new_success() {
    ...
    let state = HilbertState::<f64>::new(data.clone(), metric).unwrap();
    assert_eq!(state.mv().data(), &data);
...
fn test_new_error_data_length_mismatch() {
    ...
    let metric = Metric::Euclidean(dim);
    let err = HilbertState::<f64>::new(data, metric).unwrap_err();
    assert_eq!(
        format!("{}", err),
...
fn test_new_unchecked() {
    ...
    let data = vec![Complex::new(1.0, 0.0); size];
    let metric = Metric::Euclidean(dim);
    let state = HilbertState::<f64>::new_unchecked(data.clone(), metric);
    assert_eq!(state.mv().data(), &data);
    assert_eq!(state.mv().metric(), metric);
}
...
fn test_add_hilbert_states() {
    ...
    let state1 = HilbertState::<f64>::new(data1.clone(), metric).unwrap();
    let state2 = HilbertState::<f64>::new(data2.clone(), metric).unwrap();
...
fn test_add_hilbert_states_metric_mismatch_panics() {
    ...
    let state1 = HilbertState::<f64>::new(data1, metric1).unwrap();

    let dim = 2; // Different dimension
    let size2 = 1 << dim;
    let metric2 = Metric::Euclidean(dim);
    let data2 = vec![Complex::new(1.0, 0.0); size2];
    let state2 = HilbertState::<f64>::new(data2, metric2).unwrap();
...
fn test_mul_complex_scalar() {
    ...
    let state_complex = HilbertState::<f64>::new(data_complex.clone(), metric).unwrap();
...
#[test]
fn test_default() {
    let state = HilbertState::<f64>::default();
