...
fn test_trait_inverse_accepts_magnitude_below_epsilon() {
    ...
    let inv = <CausalMultiVector<f64> as MultiVector<f64>>::inverse(&tiny).unwrap();
    // 1/1e-9 = 1e9, to within f64 rounding of the intermediate 1e-18.
    assert!(
