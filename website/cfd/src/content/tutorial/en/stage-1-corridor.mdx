...
runtime: 44.4 s against a 600 s budget
files:
...
```text
[PASS] (5b) wall-clock budget: 44.4 s elapsed (budget 600 s)
```
...
<p>Scroll back to the line reporting blackout onset. It gives step 131 and
73.2 km. Station 2 shows that no constant in the program produced those
numbers.</p>
</div>
...
constant. The 73.2 km in the output is therefore a prediction rather than a
setting.

Consider the alternative construction, `.run_for(131)`. It reproduces today's
...
The sweep resolves to 0.5° and stops at a 2.07 m miss. Finer resolution would
