...
`.bazelignore` lists `website/cfd/node_modules` so the Rust build ignores it. The
root `MODULE.bazel` declares no `npm_*` extension, and no `BUILD.bazel` exists
...
The Worker is **`deep-causality-cfd-prod`**, hyphenated throughout;
`wrangler.toml` must carry that exact name. See
[`../README.md`](../README.md) for the two ways this fails quietly.

## Design

The binding spec is [`../web/DESIGN.md`](../web/DESIGN.md); the descriptive
companion is [`../web_design/`](../web_design/). This site follows both, with
two deliberate differences that implement spec recommendations the project
website does not yet apply:
...
re-copy it, then run `pnpm check:tokens`. Site-local tokens go in `tokens-cfd.css`.
...
- **Facts are rooted in the crate. Authoring is for the prime audience.**
  ...
  source. That constraint is absolute and makes the site citable.

  Structure, order, emphasis and vocabulary serve the prime audience: a working CFD engineer who runs Fluent, SU2,
...
- **A toolbox for a named problem class**, with an explicit line between what
  ...
  *open*. The fourth, **not pursued**, holds decisions with their reasons, so it
  carries its own status mark and nothing is promoted out of it on effort alone. An unstated non-goal
...
- **Never combine `vector-effect: non-scaling-stroke` with the draw-in.** It
  ...
  instead of a drawn one. Set `stroke-width` in user units instead. The
