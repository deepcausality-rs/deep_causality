...
| `algebra.euclidean.remainder_lt_divisor` | `0 < b → a % b < b`; the **termination argument** — φ is ℕ-valued and strictly decreases, so `gcd` is well founded | `EuclideanDomain.lean :: euclidean_remainder_lt_divisor` | `domain_euclidean_tests.rs :: test_remainder_lt_divisor` | ✓ |
