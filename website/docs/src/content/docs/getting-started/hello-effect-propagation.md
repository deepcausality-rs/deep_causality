...
Every Causaloid evaluation returns a `PropagatingEffect<T>` (stateless) or a `PropagatingProcess<T, S, C>` (stateful). Every monadic `bind` returns the same. The Causaloid page covered the structural side; the [Causal Monad page](/getting-started/hello-causal-monad/) covered the sequential side. The type that flows between them is the same on both sides.
...
The concept pages on [Causaloid](/concepts/causaloid/), [Causal Monad](/concepts/causal-monad/), and [Effect Propagation Process](/concepts/effect-propagation-process/) go deeper on the algebra. The [examples](https://www.deepcausality.com/examples/) put the composition to work.
