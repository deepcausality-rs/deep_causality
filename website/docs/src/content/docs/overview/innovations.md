...
**3. An explicit algebraic trait hierarchy.** `Magma → Semigroup → Monoid → Group → AbelianGroup`; then `Semiring → CommutativeSemiring` and `Ring → CommutativeRing → Field → RealField → ComplexField<R>`, with `EuclideanDomain` hanging off `CommutativeRing`; plus `Module<R>`, `Algebra<R>`, `AssociativeAlgebra<R>`, and `DivisionAlgebra<R>`. All five number systems sit in it — ℕ, ℤ, ℚ, ℝ, ℂ — each stopping exactly where the mathematics does: ℕ is a semiring and not a ring, ℤ is a ring and not a field, ℚ is a field and not a real. Marker traits move algebraic laws into the type system, so an algorithm that requires associativity cannot accept a type that violates it. *See [Uniform Math](/concepts/uniform-math/).*
