...
A `Context` is the explicit environment a [Causaloid](/concepts/causaloid/) reasons against. It is the half of dynamic causality that holds the world while the rules hold the structure.
...
[Causaloid](/concepts/causaloid/) is the rule that reads the Context. [Effect Propagation Process](/concepts/effect-propagation-process/) is what the rule produces. [Effect Ethos](/concepts/effect-ethos/) is what verifies the rule's output before it commits.
