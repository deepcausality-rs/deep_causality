...
- Example: [sensor monitoring with CSM](https://www.deepcausality.com/examples/sensor-monitoring-csm/), backed by [`examples/csm_examples/csm_basic`](https://github.com/deepcausality-rs/deep_causality/tree/main/examples/csm_examples/csm_basic).
- Concept: [Causaloid](/concepts/causaloid/), the unit that supplies the verdict each state acts on.
- Concept: [Effect Ethos](/concepts/effect-ethos/), for policy checks that should gate an action before it fires.
- Concept: [Uncertainty](/concepts/uncertainty/), for the hypothesis tests applied when a state's Causaloid emits an uncertain effect.
