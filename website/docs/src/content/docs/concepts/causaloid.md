...
[Context](/concepts/context/) is the structure a contextual Causaloid reads from. [Effect Propagation Process](/concepts/effect-propagation-process/) is the carrier effect that flows through a Causaloid chain. [Causal Monad](/concepts/causal-monad/) is the `pure`/`bind` algebra that carrier implements, so chains compose without losing their properties.
