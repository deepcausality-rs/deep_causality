...
`PropagatingEffect<T>` pins `State` and `Context` to `()`, so its bind threads the unit state trivially; the chain stays Markov-free. `PropagatingProcess<T, S, C>` keeps both generic, so its bind threads real state and context. They are the same algebra over the same struct. Lifting from the stateless to the stateful form is a single constructor call, [`with_state`](/concepts/effect-propagation-process/).
...
[Effect Propagation Process](/concepts/effect-propagation-process/) is the carrier this trait operates over. [HKT](/concepts/hkt/) explains how the signature stays generic across the five parameters without runtime cost. [Causaloid](/concepts/causaloid/) is what produces the values that flow through.
