...
- Evaluation uses graph methods on the [`MonadicCausableGraphReasoning`](https://github.com/deepcausality-rs/deep_causality/tree/main/deep_causality) trait, not the singleton `evaluate`. `evaluate_single_cause(idx, &effect)` runs one node. `evaluate_subgraph_from_cause`, `evaluate_shortest_path`, and similar methods drive the dynamic and adaptive reasoning modalities documented on the [Dynamic causality page](/docs/concepts/dynamic-causality/).
