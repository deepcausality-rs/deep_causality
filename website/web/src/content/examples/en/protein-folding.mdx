...
[`main.rs:36`](https://github.com/deepcausality-rs/deep_causality/blob/main/examples/medicine_examples/protein_folding/main.rs#L36). The `Probability` type from `deep_causality_physics` is a thin wrapper that constrains the inner `f64` to `[0, 1]` at construction; later steps lean on that invariant.
