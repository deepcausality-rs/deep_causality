---
title: Maxwell field derivation as a four-step CausalFlow chain
domain: Physics
summary: A plane-wave configuration becomes a vector potential, then an EM field, then a gauge check, then a Poynting flux. Four bind calls on a CausalFlow.
order: 6
category: physics
crates:
  - deep_causality
...
This example lives at [`maxwell`](https://github.com/deepcausality-rs/deep_causality/tree/main/examples/physics_examples/maxwell). It is the simplest demonstration of `CausalFlow` over a physics chain: each step is a pure function from one physical state to the next, and `bind` glues them.
