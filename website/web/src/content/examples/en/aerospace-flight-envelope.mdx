---
title: Assess stall, overspeed, and terrain risk from aircraft sensor data
domain: Aerospace
summary: Combine sensor health, state estimation, and interacting flight-envelope checks into one aircraft risk assessment.
order: 4
category: aerospace
crates:
  - deep_causality
...
This example lives at [`flight_envelope_monitor`](https://github.com/deepcausality-rs/deep_causality/tree/main/examples/avionics_examples/control/flight_envelope_monitor) in the repo. It is the most direct demonstration of `PropagatingProcess` with non-trivial state and context: five sequential `bind` calls thread a `FlightState` and an `AircraftConfig` through every step.
...
The five steps are defined as ordinary functions in the example's `model.rs`. `CausalFlow::from` lifts the seed process into the fluent API, the five `bind` calls thread the value, state, context, and log through the same five-parameter `PropagatingProcess` shape, and `into_process()` hands the raw process back for inspection.
