...
The two parts differ in status. Informational locality is structural: the signature of bind supplies a causal function with only the threaded value, state, and context, and the state-externalization discipline below requires every history-bearing dependency to travel through those channels. A host language with closures cannot by itself stop a function from capturing mutable external state, so locality is enforced by the bind interface together with that discipline rather than by the type signature alone. Stability is an interpretive commitment on the model, not a property the framework can enforce. It is the same commitment that Pearl's structural causal models make when they assume each structural equation is autonomous rather than a curve fit\cite{pearl2000causality}, and it is the modularity assumption that licenses intervention. Stability cannot be derived; it is postulated of the model by the modeller.

Mechanism autonomy is therefore a \emph{definitional postulate} rather than a second axiom. Its first part is discharged by the bind interface together with the state-externalization discipline; only its second part is assumed, and it is assumed by any causal framework whatsoever. The generalized definition of causality is, in full:
