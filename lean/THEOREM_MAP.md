...
| `core.witness.agree` | HKT witness `fmap` = inherent `fmap` on every carrier (`Some`/`None`/`Err`/**command**); functor total over commands (preserved, not collapsed); no panic (D15 retired) | proved | `DeepCausalityFormal/Core/Consistency.lean :: witness_agree / fmap_preserves_command` | `deep_causality_core/tests/formalization_lean/consistency_tests.rs :: test_witness_agrees_with_inherent_fmap` | ✓ | — | — |
