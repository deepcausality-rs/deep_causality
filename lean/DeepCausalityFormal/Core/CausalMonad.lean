...
`LawfulMonad` instance is scaling work (items 4-5).
...
/-- Left identity: `pure a >>= f = f a`.
    ...
    Because `pure'` starts with an empty log, the log-prepend in `bind'` is a no-op, so the
    result is exactly `f` applied at the injected value and state. Mirrors the Kani harness
    `causal_monad_left_identity`.

    THEOREM_MAP: `core.causal_monad.left_id` -/
theorem bind_left_id (v : V) (s : S) (f : V → S → Process W S L) :
    bind' (pure' v s) f = f v s := by
  simp [bind', pure']
