...
use deep_causality_physics::{MassFlowRate, PhysicsErrorEnum};
...
fn test_mass_flow_rate_rejects_negative() {
    assert!(
        matches!(
            MassFlowRate::<f64>::new(-1.0).unwrap_err().0,
...
fn test_mass_flow_rate_rejects_nonfinite() {
    assert!(
        matches!(
            MassFlowRate::<f64>::new(f64::NAN).unwrap_err().0,
            PhysicsErrorEnum::PhysicalInvariantBroken { .. }
        ),
        "expected a PhysicalInvariantBroken refusal"
    );
    assert!(
        matches!(
            MassFlowRate::<f64>::new(f64::INFINITY).unwrap_err().0,
            PhysicsErrorEnum::PhysicalInvariantBroken { .. }
        ),
        "expected a PhysicalInvariantBroken refusal"
    );
