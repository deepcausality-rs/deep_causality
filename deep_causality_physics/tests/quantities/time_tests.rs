...
fn test_time_new_unchecked() {
    let t = Time::<f64>::new_unchecked(100.0);
    assert!((t.value() - 100.0).abs() < 1e-10);
}

#[test]
fn test_time_default() {
    let t = Time::<f64>::default();
...
fn test_time_from_minutes() {
    // 1 minute = 60 seconds
    let t = Time::<f64>::from_minutes(1.0).unwrap();
...
fn test_time_from_hours() {
    // 1 hour = 3600 seconds
    let t = Time::<f64>::from_hours(1.0).unwrap();
...
fn test_time_from_days() {
    // 1 day = 86400 seconds
    let t = Time::<f64>::from_days(1.0).unwrap();
