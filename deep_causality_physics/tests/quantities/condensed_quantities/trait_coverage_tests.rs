...
fn test_condensed_scalars_traits() {
    ...
    assert_eq!(m, m.clone());
    assert!(m < Mobility::<f64>::new(2.0).unwrap());
    let _ = format!("{:?}", m);

    let ta = TwistAngle::<f64>::new(0.5).unwrap();
    assert_eq!(ta, ta.clone());
    assert!(ta < TwistAngle::<f64>::new(1.0).unwrap());
    let _ = format!("{:?}", ta);
}
