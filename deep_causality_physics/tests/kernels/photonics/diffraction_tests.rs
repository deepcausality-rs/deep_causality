...
fn test_single_slit() {
    let i0 = 1.0;
    let a = Length::<f64>::new(1.0).unwrap();
    let lambda = Wavelength::<f64>::new(1.0).unwrap();

    // Theta = 0 -> beta = 0 -> sinc(0)=1 -> I=I0
    let theta0 = RayAngle::<f64>::new(0.0).unwrap();
    ...
    let theta_min = RayAngle::<f64>::new(PI / 2.0).unwrap();
...
fn test_grating() {
    ...
    let d = Length::<f64>::new(2.0).unwrap();
    let m = 1;
    let lambda = Wavelength::<f64>::new(1.0).unwrap();
    let inc = RayAngle::<f64>::new(0.0).unwrap();
...
fn test_single_slit_errors() {
    let a = Length::<f64>::new(1.0).unwrap();
    let theta = RayAngle::<f64>::new(0.0).unwrap();
...
fn test_grating_errors() {
    let pitch_valid = Length::<f64>::new(1.0).unwrap();
    let lambda = Wavelength::<f64>::new(1.0).unwrap();
    let inc = RayAngle::<f64>::new(0.0).unwrap();
