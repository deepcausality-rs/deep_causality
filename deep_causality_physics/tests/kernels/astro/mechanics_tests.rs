...
fn test_orbital_velocity_kernel_valid() {
    ...
    let mass = Mass::<f64>::new(1e24).unwrap(); // kg
    let radius = Length::<f64>::new(6.371e6).unwrap(); // Earth radius in meters
...
fn test_orbital_velocity_kernel_zero_mass() {
    ...
    let radius = Length::<f64>::new(1000.0).unwrap();
...
fn test_escape_vs_orbital_velocity_invariant() {
    let mass = Mass::<f64>::new(5.972e24).unwrap(); // Earth mass
    let radius = Length::<f64>::new(6.371e6).unwrap(); // Earth radius
...
fn test_schwarzschild_radius_kernel_valid() {
    let mass = Mass::<f64>::new(1.989e30).unwrap(); // Solar mass
...
fn test_schwarzschild_radius_sun_known_value() {
    ...
    let solar_mass = Mass::<f64>::new(1.989e30).unwrap();
