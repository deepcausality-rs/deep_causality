...
fn test_energy_from_electron_volts() {
    let e = Energy::<f64>::from_electron_volts(1.0).unwrap();
    // 1 eV = 1.602e-19 J
...
fn test_energy_from_calories() {
    let e = Energy::<f64>::from_calories(1.0).unwrap();
    // 1 cal = 4.184 J
...
fn test_energy_from_kilowatt_hours() {
    let e = Energy::<f64>::from_kilowatt_hours(1.0).unwrap();
    // 1 kWh = 3.6e6 J
...
fn test_energy_as_electron_volts_roundtrip() {
    let e = Energy::<f64>::from_electron_volts(1000.0).unwrap();
...
fn test_energy_as_calories_roundtrip() {
    let e = Energy::<f64>::from_calories(100.0).unwrap();
...
fn test_energy_as_kilowatt_hours_roundtrip() {
    let e = Energy::<f64>::from_kilowatt_hours(2.0).unwrap();
...
fn test_energy_into_f64() {
    let e = Energy::<f64>::new_unchecked(42.0);
    let val: f64 = e.into();
