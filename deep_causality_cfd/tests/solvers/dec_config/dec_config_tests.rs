...
//! Tests for the DEC NS type-state config builder: the optional knobs, the validated getters, and
...
use deep_causality_cfd::CfdConfigBuilder;
...
fn test_non_finite_viscosity_is_numerical_instability() {
    let err = CfdConfigBuilder::dec_ns()
        .viscosity(f64::NAN)
        .time_step(0.01)
        .build()
