...
Runnable **verification** programs for the CFD stack, covering the DEC-native solver and the
...
reference check fails, so the suite works as a gate. The per-example sections below state what each
one checks and how it fails.
...
those three lists, so no harness can be added without being scheduled. `cargo test` only *compiles*
these binaries; this workflow is what enforces the quantitative claims on this page.
...
stderr together**, holding both the reported quantities and the gate block. A baseline must reach the
harness's terminal summary; a truncated or aborted run is not committed, because it removes the
reference a reader compares against.
...
- **A failing baseline is committed as failing.** `qtt_cylinder_verification`'s committed baseline is
  from an `L = 5` configuration (`exit 1`, two `NOT CONVERGING` ladder verdicts). The harness runs at
  `L = 8` with a wall-error-target `η` (`close-qtt-solver-envelope`). A single march takes ~17 min and
  the full harness ~4-9 hours, so it is **offline / manual, not in CI** (see the OFFLINE / MANUAL note
  in `.github/workflows/cfd_verification.yml`). The `L = 8` baseline is **pending an offline
  regeneration**; the `L = 5` artifact stays as the last completed run. The gate stays red for a
...
> ladders gate, and in the committed `L = 5` baseline both report `NOT CONVERGING`. This is a correct
...
>
> Root cause at `L = 5`: the physical Brinkman layer `√(ην) = 0.144·dx` is ~7× thinner than one cell, and the
...
>
> **What the bond ladder shows.** Convergence of the *bond* ladder is saturation of the tensor-train
> compression. It says nothing about whether the compressed quantity is a drag. Read the absolute
> `C_d ≈ 23.8` as a property of this configuration's blur width, not of a cylinder.
>
...
> The gate stays: widening the bound until it passes would be back-fitting.
...
**6.66e-16**: **0 %** divergence, at the floor of the working precision.
...
**< 1 %** from order 2 at the finest pair. Error magnitudes at 64²: ≈ 5e-3 (convective),
...
because 16³ is under-resolved and cannot represent the small-scale dissipation peak. The
...
**Verifies.** *Internal consistency only.* This confined, periodic-x harness (a prescribed moving wall,
not a true inflow/outflow surface) exercises the cut-cell + uncertain-zone machinery and makes
**no quantitative reference claim** for the shedding Strouhal. The gates are (a) incompressibility (the
...
produced a cycle-mean drag, and `St` and `C_d` each sit inside a pinned band. A solver error
**exits 1** instead of breaking the march and reporting `St`/`C_d` from the truncated series.

All four bounds are `[tripwire]`, not `[reference]`. At the affordable 8 cells/D
...
Convection vs the closed form **3.2e-3** (≈ 0.6 % of the 0.5 signal): the nonlinear term is present
and correct. Divergence **~1e-14** (the spectral Leray projection is exact to machine precision). Bond `= N`
...
integration is a separate derivation; the tolerance is sized from the reference's own
