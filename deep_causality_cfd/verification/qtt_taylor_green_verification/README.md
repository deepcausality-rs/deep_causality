...
This example verifies the `QttIncompressible2d` solver against the closed-form 2-D **Taylor–Green
vortex** (Taylor & Green, 1937), the standard analytic reference for a periodic incompressible
solver. The solver's 2-D incompressible Navier–Stokes flowfield lives and evolves entirely as a
**tensor train** (the CFD ↔ tensor-network bridge). The **CfdFlow** DSL (`CfdFlow::march`) drives
the whole rollout, so the example also exercises the DSL wiring and observable extraction.
...
gradient**, absorbed entirely by the pressure. The incompressible dynamics therefore reduces to pure
diffusion, and the decay rate is analytic.
...
2. **Correct nonlinear convection.** Single-mode Taylor–Green's convective term is a *pure gradient
   the projection removes*, so the marched decay is **insensitive to whether convection is computed
   correctly**: a solver with a broken (or zero) `u·∇u` would still pass gate 1. The harness therefore
   checks the nonlinear term **directly**, through the **shipped** convection path. It calls the
   solver's own `rate_pair` on a zero-viscosity instance, which returns the pure convection
   `−(u·∇)u`; that must reproduce the closed form `−½ sin(2x)` with a small error and a **non-zero**
   amplitude. A re-assembled copy of the operator would pass even if the marcher's own convection
   were wrong.

3. **Incompressibility.** The post-projection divergence residual stays at the projection floor.

4. **MPS compression.** The harness reports the headline tensor-network metric, maximum bond
   dimension vs. dense element count, per level.
...
- **Temporal floor, and the ladder's maximum usable length.** Because `dt` is held fixed, the
  first-order Euler error is a floor under the total error, with the **opposite sign** to the
  spatial error. The signed error of the decay factor runs `+9.8e-4, +2.4e-4, +5.3e-5, +5.9e-6,
  −5.9e-6, −8.8e-6` for `N = 8 … 256`: the two contributions **cancel** near `N = 64–128`. The
  `N = 64` observed order of **3.16 is a cancellation artifact, not super-convergence**, and by
  `N = 128` the order collapses to **0.02**. **Use `max_level = 5` (the committed default);** at
  `max_level 7` the finest-pair order falls below `MIN_ORDER` and the harness **fails**. A longer
  ladder needs a two-sided order gate or `dt` refined as `dt ∝ dx²`.
- **Convection:** `u·∇u` vs `−½ sin(2x)`: max abs error **3.2e-3** at 32² (the centered-difference
  truncation of `∂ₓ` on the field) against a signal amplitude of 0.5, so the nonlinear term is
  **real and correct**.
...
- **Compression:** bond `= N` here (a smooth low-frequency field), so storage is `O(N)` vs the dense
  `O(N²)`, an `N×` compression that grows with resolution.
...
discretization error and its convergence order, the standard of code verification. Gate 2 closes the
convection-masking gap that single-mode TG leaves, so a passing run exercises the nonlinear term as
well as diffusion + projection.

This is a **periodic, smooth, low-Reynolds, single-mode** case. It does **not** test immersed-body
boundary conditions (`qtt_cylinder_verification` covers those), turbulent rank growth, or multi-mode
energy cascade. The rank-vs-accuracy curve of Peddinti et al. / Gourianov et al. is the headline
metric for those anchors.
...
`max_level` (default 5) extends the ladder to a `2^max_level` grid. stdout carries the labeled report
and the closing verdict; stderr carries any gate's `FAIL:` line, and the process exits nonzero when a
gate breaks.
