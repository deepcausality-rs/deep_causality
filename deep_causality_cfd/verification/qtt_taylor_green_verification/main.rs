...
/// `u`-component against the closed form `−½ sin(2x)`. Returns `(max_abs_err, signal_amp)` — a small
/// error with non-zero amplitude proves the nonlinear term is real and correct. (Single-mode TG's
/// convective term is a pure gradient the projection removes, so the marched decay cannot test it.)
///
/// # Errors
/// Any codec / operator failure.
fn convection_operator_error(l: usize) -> Result<(f64, f64), PhysicsError> {
    let n = 1usize << l;
