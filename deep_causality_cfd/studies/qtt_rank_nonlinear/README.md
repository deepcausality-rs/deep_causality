...
**What it tests.** `qtt_rank_dynamic` leaves one case open. Its `QttLinear1d` vehicle is *linear*, so
...
marcher** from the QTT primitives, `u_t + ½(u²)_x = ν u_xx` in conservative flux form with
...
- **A 1-D forming shock is cheap.** `u₀ = 0.5 + 0.4 sin(2πx)` steepens into a shock, and the bond
  peaks at **8** and stays there. Nonlinear steepening does not blow up 1-D rank, as the static
  study predicts: a 1-D discontinuity is rank 2 or less.
- **A 2-D forming *curved* shock raises rank, so the threat is real and dynamic.** A smooth radial
  ...
  with resolution. This confirms the static 2-D result dynamically.
- **Thickening could NOT be tested as a lever here.** The explicit 2-D diffusion limit
  ...
  stable window, not about thickness as a rank lever: an explicit scheme cannot raise artificial
  viscosity at will.
...
measure the cost of doing so. `qtt_rank_fitted_dynamic` runs the shock-aligned confinement test;
both levers are Tier-B design commitments. Analysis: `openspec/notes/archive/cfd-plasma-blackout/gap-2/`.

**Caveats.** Burgers is a scalar model, not the compressible Euler/NS system. Explicit Euler with
central differences is the minimal shock-former, not a production scheme. 64² and 128² are
